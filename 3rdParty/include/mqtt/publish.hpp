...
constexpr bool is_dup(std::uint8_t v) { return (v & 0b00001000) != 0; }

constexpr qos get_qos(std::uint8_t v) {
    return static_cast<qos>((v & 0b00000110) >> 1);
}

constexpr bool is_retain(std::uint8_t v) { return (v & 0b00000001) != 0; }

constexpr void set_dup(std::uint8_t& fixed_header, bool dup) {
    if (dup)
        fixed_header |= 0b00001000;
    else
        fixed_header &= static_cast<std::uint8_t>(~0b00001000);
}
...
struct publish_options final {
    ...
    constexpr publish_options(retain value)
        : data_(static_cast<std::uint8_t>(value)) {}
    ...
    constexpr publish_options(qos value)
        ...
        BOOST_ASSERT(value == qos::at_most_once ||
                     value == qos::at_least_once || value == qos::exactly_once);
    }

    constexpr publish_options operator|(publish_options rhs) const {
        return publish_options(data_ | rhs.data_);
    }
    constexpr publish_options operator|(retain rhs) const {
        return *this | publish_options(rhs);
    }
   ...
   private:
    std::uint8_t data_ =
        0;  // defaults to retain::no, dup::no, qos::at_most_once
};

constexpr publish_options operator|(retain lhs, dup rhs) {
    return publish_options(lhs) | rhs;
}
...
inline std::ostream& operator<<(std::ostream& os, retain val) {
...
inline std::ostream& operator<<(std::ostream& os, dup val) {
    os << dup_to_str(val);
    return os;
}
