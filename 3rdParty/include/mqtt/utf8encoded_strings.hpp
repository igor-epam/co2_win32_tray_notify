...
enum struct validation {
    ...
    /**
     * @brief UTF-8 string is well_formed and contains control character and
     * non-character. See
     * http://docs.oasis-open.org/mqtt/mqtt/v3.1.1/os/mqtt-v3.1.1-os.html#_Toc398718016
     * 1.5.3 UTF-8 encoded strings
     */
...
constexpr bool is_valid_length(string_view str) { return str.size() <= 0xffff; }
...
constexpr validation validate_contents(string_view str) {
    ...
    while (it != end) {
        if (static_cast<unsigned char>(*(it + 0)) < 0b1000'0000) {
            ...
            ++it;
        } else if ((static_cast<unsigned char>(*(it + 0)) & 0b1110'0000) ==
                   0b1100'0000) {
            // 110XXXXx 10xxxxxx
            if (it + 1 >= end) {
                result = validation::ill_formed;
            ...
            if ((static_cast<unsigned char>(*(it + 1)) & 0b1100'0000) !=
                    0b1000'0000 ||
                (static_cast<unsigned char>(*(it + 0)) & 0b1111'1110) ==
                    0b1100'0000) {  // overlong
                result = validation::ill_formed;
                break;
            }
            ...
            it += 2;
        } else if ((static_cast<unsigned char>(*(it + 0)) & 0b1111'0000) ==
                   0b1110'0000) {
            ...
            if ((static_cast<unsigned char>(*(it + 1)) & 0b1100'0000) !=
                    0b1000'0000 ||
                (static_cast<unsigned char>(*(it + 2)) & 0b1100'0000) !=
                    0b1000'0000 ||
                (static_cast<unsigned char>(*(it + 0)) == 0b1110'0000 &&
                    (static_cast<unsigned char>(*(it + 1)) & 0b1110'0000) ==
                        0b1000'0000) ||  // overlong?
                (static_cast<unsigned char>(*(it + 0)) == 0b1110'1101 &&
                    (static_cast<unsigned char>(*(it + 1)) & 0b1110'0000) ==
                        0b1010'0000)) {  // surrogate?
                result = validation::ill_formed;
                break;
            }
            if (static_cast<unsigned char>(*(it + 0)) == 0b1110'1111 &&
                static_cast<unsigned char>(*(it + 1)) == 0b1011'1111 &&
                (static_cast<unsigned char>(*(it + 2)) & 0b1111'1110) ==
                    0b1011'1110) {
                // U+FFFE or U+FFFF?
                result = validation::well_formed_with_non_charactor;
            }
            it += 3;
        } else if ((static_cast<unsigned char>(*(it + 0)) & 0b1111'1000) ==
                   0b1111'0000) {
