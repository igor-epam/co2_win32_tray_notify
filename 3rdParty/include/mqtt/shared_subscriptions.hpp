...
inline bool operator<(
    share_name_topic_filter const& lhs, share_name_topic_filter const& rhs) {
...
inline bool operator==(
    share_name_topic_filter const& lhs, share_name_topic_filter const& rhs) {
    return lhs.share_name == rhs.share_name &&
           lhs.topic_filter == rhs.topic_filter;
}
...
inline optional<share_name_topic_filter> parse_shared_subscription(
    buffer whole_topic_filter) {
    ...
    if (whole_topic_filter.substr(0, shared_prefix.size()) != shared_prefix) {
        return share_name_topic_filter{
            buffer{}, force_move(whole_topic_filter)};
    }

    // Remove $share/
    whole_topic_filter.remove_prefix(shared_prefix.size());

    // This is the '/' seperating the subscription group from the actual
    // topic_filter.
    auto const idx = whole_topic_filter.find_first_of('/');
    if (idx == string_view::npos) return nullopt;

    // We return the share_name and the topic_filter as buffers that point to
    // the same storage. So we grab the substr for "share", and then remove it
    // from whole_topic_filter.
    auto share_name = whole_topic_filter.substr(0, idx);
    whole_topic_filter.remove_prefix(
        std::min(idx + 1, whole_topic_filter.size()));

    if (share_name.empty() || whole_topic_filter.empty()) return nullopt;
    return share_name_topic_filter{
        force_move(share_name), force_move(whole_topic_filter)};
}

namespace detail {

template <typename T, typename U>
inline buffer create_topic_filter_buffer(
    T const& share_name, U const& topic_filter) {
    string_view prefix = "$share/";
    // 1 is the length of '/' between share_name and topic_filter
    auto spa = make_shared_ptr_array(
        prefix.size() + share_name.size() + 1 + topic_filter.size());
    auto it = spa.get();
    auto start = it;
    ...
    *it++ = '/';
    ...
    return buffer(string_view(start, static_cast<std::size_t>(it - start)),
        force_move(spa));
}
