...
inline boost::container::static_vector<char, 4> num_to_4bytes(
    std::uint32_t val) {
    return {static_cast<char>(val >> 24), static_cast<char>(val >> 16),
        static_cast<char>(val >> 8), static_cast<char>(val & 0xff)};
}
