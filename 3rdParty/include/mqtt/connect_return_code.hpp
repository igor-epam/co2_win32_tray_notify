...
constexpr char const* connect_return_code_to_str(connect_return_code v) {
    char const* const str[] = {"accepted", "unacceptable_protocol_version",
        "identifier_rejected", "server_unavailable",
        "bad_user_name_or_password", "not_authorized"};
    if (static_cast<std::uint8_t>(v) < sizeof(str))
        return str[static_cast<std::uint8_t>(v)];
    return "unknown_connect_return_code";
}

inline std::ostream& operator<<(std::ostream& os, connect_return_code val) {
