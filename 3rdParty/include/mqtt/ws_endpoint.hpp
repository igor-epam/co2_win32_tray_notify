...
class ws_endpoint : public socket {
   public:
    ...
    MQTT_ALWAYS_INLINE void async_read(as::mutable_buffer buffers,
        std::function<void(error_code, std::size_t)> handler) override final {
        ...
        beast_read_handler.reset(new beast_read_handler_t(
            [this, req_size, buffers, handler = force_move(handler)](
                ...
                if (!ws_.got_binary()) {
                    ...
                    force_move(handler)(boost::system::errc::make_error_code(
                                            boost::system::errc::bad_message),
                        0);
                    return;
                }
                if (req_size > buffer_.size()) {
                    auto beast_read_handler =
                        std::static_pointer_cast<beast_read_handler_t>(v);
                    ws_.async_read(buffer_,
                        as::bind_executor(strand_,
                            [beast_read_handler](error_code ec, std::size_t) {
        ...
        ws_.async_read(
            buffer_, as::bind_executor(strand_,
                         [beast_read_handler](error_code ec, std::size_t) {
                             (*beast_read_handler)(ec, beast_read_handler);
                         }));
    }

    MQTT_ALWAYS_INLINE void async_write(std::vector<as::const_buffer> buffers,
        std::function<void(error_code, std::size_t)> handler) override final {
        ws_.async_write(
            buffers, as::bind_executor(strand_, force_move(handler)));
    }

    MQTT_ALWAYS_INLINE std::size_t write(std::vector<as::const_buffer> buffers,
        boost::system::error_code& ec) override final {
        ws_.write(buffers, ec);
        return as::buffer_size(buffers);
    }

    MQTT_ALWAYS_INLINE void post(std::function<void()> handler) override final {
        as::post(strand_, force_move(handler));
    }

#if BOOST_VERSION >= 107000

    MQTT_ALWAYS_INLINE as::ip::tcp::socket::lowest_layer_type& lowest_layer()
        override final {
    ...
    MQTT_ALWAYS_INLINE void clean_shutdown_and_close(
        boost::system::error_code& ec) override final {
        if (ws_.is_open()) {
            // WebSocket closing process
            MQTT_LOG("mqtt_impl", trace)
                << MQTT_ADD_VALUE(address, this) << "call beast close";
            ws_.close(boost::beast::websocket::close_code::normal, ec);
            MQTT_LOG("mqtt_impl", trace) << MQTT_ADD_VALUE(address, this)
                                         << "ws close ec:" << ec.message();
            if (!ec) {
                do {
                ...
                MQTT_LOG("mqtt_impl", trace) << MQTT_ADD_VALUE(address, this)
                                             << "ws read ec:" << ec.message();
            }
        }
        shutdown_and_close_impl(next_layer(), ec);
    }

    MQTT_ALWAYS_INLINE void async_clean_shutdown_and_close(
        std::function<void(error_code)> handler) override final {
        if (ws_.is_open()) {
            // WebSocket closing process
            MQTT_LOG("mqtt_impl", trace)
                << MQTT_ADD_VALUE(address, this) << "call beast async_close";
            ws_.async_close(boost::beast::websocket::close_code::normal,
                as::bind_executor(
                    strand_, [this, handler = force_move(handler)](
                                 error_code ec) mutable {
                        if (ec) {
                            MQTT_LOG("mqtt_impl", trace)
                                << MQTT_ADD_VALUE(address, this)
                                << "ws async_close ec:" << ec.message();
                            async_shutdown_and_close_impl(
                                next_layer(), force_move(handler));
                        } else {
    ...
    MQTT_ALWAYS_INLINE void force_shutdown_and_close(
        boost::system::error_code& ec) override final {
    ...
    typename boost::beast::websocket::stream<Socket>::next_layer_type&
    next_layer() {
    ...
    void async_accept(
        ConstBufferSequence const& buffers, AcceptHandler&& handler) {
        ws_.async_accept(buffers, std::forward<AcceptHandler>(handler));
    }

    template <typename ConstBufferSequence, typename ResponseDecorator,
        typename AcceptHandler>
    void async_accept_ex(ConstBufferSequence const& buffers,
        ResponseDecorator const& decorator, AcceptHandler&& handler) {
        ws_.async_accept_ex(
            buffers, decorator, std::forward<AcceptHandler>(handler));
    }
   ...
   private:
    void async_read_until_closed(std::function<void(error_code)> handler) {
        ...
        ws_.async_read(*buffer,
            as::bind_executor(strand_, [this, handler = force_move(handler)](
    ...
    void shutdown_and_close_impl(as::basic_socket<boost::asio::ip::tcp>& s,
        boost::system::error_code& ec) {
        s.shutdown(as::ip::tcp::socket::shutdown_both, ec);
        MQTT_LOG("mqtt_impl", trace)
            << MQTT_ADD_VALUE(address, this) << "shutdown ec:" << ec.message();
        s.close(ec);
        MQTT_LOG("mqtt_impl", trace)
            << MQTT_ADD_VALUE(address, this) << "close ec:" << ec.message();
    }

    void async_shutdown_and_close_impl(
        as::basic_socket<boost::asio::ip::tcp>& s,
        std::function<void(error_code)> handler) {
        post([this, &s, handler = force_move(handler)]() mutable {
            error_code ec;
    ...
    void shutdown_and_close_impl(
        tls::stream<as::ip::tcp::socket>& s, boost::system::error_code& ec) {
        s.shutdown(ec);
        MQTT_LOG("mqtt_impl", trace)
            << MQTT_ADD_VALUE(address, this) << "shutdown ec:" << ec.message();
        shutdown_and_close_impl(lowest_layer(), ec);
    }
    void async_shutdown_and_close_impl(tls::stream<as::ip::tcp::socket>& s,
        std::function<void(error_code)> handler) {
        s.async_shutdown(as::bind_executor(strand_,
            [this, handler = force_move(handler)](error_code ec) mutable {
                MQTT_LOG("mqtt_impl", trace) << MQTT_ADD_VALUE(address, this)
                                             << "shutdown ec:" << ec.message();
                shutdown_and_close_impl(lowest_layer(), ec);
                force_move(handler)(ec);
            }));
    }
