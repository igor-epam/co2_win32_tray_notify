 ...
 */
inline void setup_log(std::map<std::string, severity_level> threshold) {
    // https://www.boost.org/doc/libs/1_73_0/libs/log/doc/html/log/tutorial/advanced_filtering.html

    auto fmt = [](boost::log::record_view const& rec,
                   boost::log::formatting_ostream& strm) {
        // Timestamp custom formatting example
        if (auto v = boost::log::extract<boost::posix_time::ptime>(
                "TimeStamp", rec)) {
            strm.imbue(std::locale(strm.getloc(),
                // https://www.boost.org/doc/html/date_time/date_time_io.html#date_time.format_flags
                new boost::posix_time::time_facet(
                    "%H:%M:%s")  // ownership is moved here
                ));
            strm << v.get() << " ";
        }
        // ThreadID example
        if (auto v =
                boost::log::extract<boost::log::thread_id>("ThreadID", rec)) {
            strm << "T:" << v.get() << " ";
        }
    ...
    auto fil = [threshold = force_move(threshold)](
 ...
 *        If the log severity_level >= threshold then log message outputs.
 */
inline void setup_log(severity_level threshold = severity_level::warning) {
    setup_log({
        {"mqtt_api", threshold},
        {"mqtt_cb", threshold},
        {"mqtt_impl", threshold},
