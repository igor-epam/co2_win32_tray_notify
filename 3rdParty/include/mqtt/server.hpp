...
template <typename Strand = strand, typename Mutex = std::mutex,
...
class server {
   public:
    ...
    using endpoint_t =
        callable_overlay<server_endpoint<Mutex, LockGuard, PacketIdBytes>>;
    ...
    server(AsioEndpoint&& ep, as::io_context& ioc_accept,
        as::io_context& ioc_con, AcceptorConfig&& config)
    ...
    server(
        AsioEndpoint&& ep, as::io_context& ioc_accept, as::io_context& ioc_con)
        : server(std::forward<AsioEndpoint>(ep), ioc_accept, ioc_con,
              [](as::ip::tcp::acceptor&) {}) {}

    template <typename AsioEndpoint, typename AcceptorConfig>
    server(AsioEndpoint&& ep, as::io_context& ioc, AcceptorConfig&& config)
        : server(std::forward<AsioEndpoint>(ep), ioc, ioc,
              std::forward<AcceptorConfig>(config)) {}
    ...
    template <typename AsioEndpoint, typename AcceptorConfig>
    server(
        AsioEndpoint&& ep, as::io_context& ioc_accept,
    ...
    void listen() {
        ...
        if (!acceptor_) {
            try {
                ...
                as::post(ioc_accept_, [this, ec = e.code()] {
    ...
    void close() {
        close_request_ = true;
        as::post(ioc_accept_, [this] { acceptor_.reset(); });
    }
     ...
     * If the version is set to protocol_version::undetermined, all versions are
     * accepted. Initial value is protocol_version::undetermined.
     */
   ...
   private:
    void do_accept() {
        ...
        acceptor_.value().async_accept(socket->lowest_layer(),
            [this, socket, &ioc_con](error_code ec) mutable {
                if (ec) {
                    acceptor_.reset();
                    if (h_error_) h_error_(ec);
                    return;
                }
                auto sp = std::make_shared<endpoint_t>(
                    ioc_con, force_move(socket), version_);
...
class server_tls {
   public:
    ...
    template <typename AsioEndpoint, typename AcceptorConfig>
    server_tls(AsioEndpoint&& ep, tls::context&& ctx,
        as::io_context& ioc_accept, as::io_context& ioc_con,
    ...
    server_tls(AsioEndpoint&& ep, tls::context&& ctx,
        as::io_context& ioc_accept, as::io_context& ioc_con)
        : server_tls(std::forward<AsioEndpoint>(ep), force_move(ctx),
              ioc_accept, ioc_con, [](as::ip::tcp::acceptor&) {}) {}

    template <typename AsioEndpoint, typename AcceptorConfig>
    server_tls(AsioEndpoint&& ep, tls::context&& ctx, as::io_context& ioc,
        AcceptorConfig&& config)
        : server_tls(std::forward<AsioEndpoint>(ep), force_move(ctx), ioc, ioc,
              std::forward<AcceptorConfig>(config)) {}
    ...
    template <typename AsioEndpoint, typename AcceptorConfig>
    server_tls(
        AsioEndpoint&& ep, tls::context&& ctx, as::io_context& ioc_accept,
        std::function<as::io_context&()> ioc_con_getter,
        AcceptorConfig&& config = [](as::ip::tcp::acceptor&) {})
        : ep_(std::forward<AsioEndpoint>(ep)),
          ioc_accept_(ioc_accept),
     ...
     * If the timer is fired, the endpoint is removed, the socket is
     * automatically closed. The default timeout value is 10 seconds.
     * @param timeout timeout value
     */
    void set_underlying_connect_timeout(
     ...
     * @return ssl context
     */
    tls::context& get_ssl_context() { return ctx_; }

    /**
     * @brief Get boost asio ssl context.
     * @return ssl context
     */
    tls::context const& get_ssl_context() const { return ctx_; }
   ...
   private:
    void do_accept() {
        ...
        acceptor_.value().async_accept(
            ps->lowest_layer(), [this, socket = force_move(socket), &ioc_con](
                ...
                tim->async_wait(
                    [socket, tim, underlying_finished](error_code ec) {
                ...
                auto ps = socket.get();
                ps->async_handshake(tls::stream_base::server,
                    [this, socket = force_move(socket), tim,
                        underlying_finished, &ioc_con](error_code ec) mutable {
   ...
   private:
    ...
    tls::context ctx_;
    protocol_version version_ = protocol_version::undetermined;
    std::chrono::steady_clock::duration underlying_connect_timeout_ =
...
class server_ws {
   public:
    ...
    template <typename AsioEndpoint, typename AcceptorConfig>
    server_ws(AsioEndpoint&& ep, as::io_context& ioc_accept,
        as::io_context& ioc_con, AcceptorConfig&& config)
        : ep_(std::forward<AsioEndpoint>(ep)),
          ioc_accept_(ioc_accept),
          ioc_con_(&ioc_con),
    ...
    server_ws(
        AsioEndpoint&& ep, as::io_context& ioc_accept, as::io_context& ioc_con)
        : server_ws(std::forward<AsioEndpoint>(ep), ioc_accept, ioc_con,
              [](as::ip::tcp::acceptor&) {}) {}

    template <typename AsioEndpoint, typename AcceptorConfig>
    server_ws(AsioEndpoint&& ep, as::io_context& ioc, AcceptorConfig&& config)
        : server_ws(std::forward<AsioEndpoint>(ep), ioc, ioc,
              std::forward<AcceptorConfig>(config)) {}

    template <typename AsioEndpoint>
    server_ws(AsioEndpoint&& ep, as::io_context& ioc)
        : server_ws(std::forward<AsioEndpoint>(ep), ioc, ioc,
              [](as::ip::tcp::acceptor&) {}) {}
   ...
   private:
    void do_accept() {
        ...
        auto socket = std::make_shared<socket_t>(ioc_con);
        auto ps = socket.get();
        acceptor_.value().async_accept(ps->next_layer(), [this,
                                                             socket =
                                                                 force_move(
                                                                     socket),
                                                             &ioc_con](
            ...
            auto tim = std::make_shared<as::steady_timer>(ioc_con);
            tim->expires_after(underlying_connect_timeout_);
            tim->async_wait([socket, tim, underlying_finished](error_code ec) {
                if (*underlying_finished) return;
                if (ec) return;
                socket->post([socket] {
                    boost::system::error_code close_ec;
            ...
            auto ps = socket.get();
            boost::beast::http::async_read(ps->next_layer(), *sb, *request,
                [this, socket = force_move(socket), sb, request, tim,
                    underlying_finished,
                    &ioc_con](error_code ec, std::size_t) mutable {
                    if (ec) {
                        *underlying_finished = true;
                        tim->cancel();
                        return;
                    ...
                    if (it != request->end()) {
                        ps->set_option(
                            boost::beast::websocket::stream_base::decorator(
                                [name = it->name(),
                                    value =
                                        it->value()]  // name is enum, value is
                                                      // boost::string_view
                                (boost::beast::websocket::response_type & res) {
                                    // This lambda is called before the scope
                                    // out point *1
                                    res.set(name, value);
                                }));
                    }
                    ps->async_accept(
                        *request, [this, socket = force_move(socket), tim,
                                      underlying_finished,
                                      &ioc_con](error_code ec) mutable {
                            *underlying_finished = true;
                            tim->cancel();
                            if (ec) {
                                return;
...
class server_tls_ws {
   public:
    ...
    template <typename AsioEndpoint, typename AcceptorConfig>
    server_tls_ws(AsioEndpoint&& ep, tls::context&& ctx,
        as::io_context& ioc_accept, as::io_context& ioc_con,
        AcceptorConfig&& config)
        : ep_(std::forward<AsioEndpoint>(ep)),
          ioc_accept_(ioc_accept),
    ...
    server_tls_ws(AsioEndpoint&& ep, tls::context&& ctx,
        as::io_context& ioc_accept, as::io_context& ioc_con)
        : server_tls_ws(std::forward<AsioEndpoint>(ep), force_move(ctx),
              ioc_accept, ioc_con, [](as::ip::tcp::acceptor&) {}) {}

    template <typename AsioEndpoint, typename AcceptorConfig>
    server_tls_ws(AsioEndpoint&& ep, tls::context&& ctx, as::io_context& ioc,
        AcceptorConfig&& config)
        : server_tls_ws(std::forward<AsioEndpoint>(ep), force_move(ctx), ioc,
              ioc, std::forward<AcceptorConfig>(config)) {}

    template <typename AsioEndpoint>
    server_tls_ws(AsioEndpoint&& ep, tls::context&& ctx, as::io_context& ioc)
        : server_tls_ws(std::forward<AsioEndpoint>(ep), force_move(ctx), ioc,
              ioc, [](as::ip::tcp::acceptor&) {}) {}
   ...
   private:
    void do_accept() {
        ...
        auto socket = std::make_shared<socket_t>(ioc_con, ctx_);
        auto ps = socket.get();
        acceptor_.value().async_accept(ps->next_layer().next_layer(),
            [this, socket = force_move(socket), &ioc_con](
                ...
                auto ps = socket.get();
                ps->next_layer().async_handshake(tls::stream_base::server,
                    [this, socket = force_move(socket), tim,
                        underlying_finished, &ioc_con](error_code ec) mutable {
                        if (ec) {
                            *underlying_finished = true;
                        ...
                        auto sb = std::make_shared<boost::asio::streambuf>();
                        auto request =
                            std::make_shared<boost::beast::http::request<
                                boost::beast::http::string_body>>();
                        auto ps = socket.get();
                        boost::beast::http::async_read(ps->next_layer(), *sb,
                            *request,
                            [this, socket = force_move(socket), sb, request,
                                tim, underlying_finished,
                                &ioc_con](error_code ec, std::size_t) mutable {
                                if (ec) {
                                    *underlying_finished = true;
                                    tim->cancel();
                                ...
                                auto it =
                                    request->find("Sec-WebSocket-Protocol");
                                if (it != request->end()) {
                                    ps->set_option(boost::beast::websocket::
                                            stream_base::decorator(
                                                ...
                                                (boost::beast::websocket::
                                                    ...
                                                    // before the scope out
                                                    // point *1
                                                    res.set(name, value);
                                                }));
                                }
                                ps->async_accept(*request,
                                    [this, socket = force_move(socket), tim,
                                        underlying_finished,
                                        &ioc_con](error_code ec) mutable {
                                        *underlying_finished = true;
                                        ...
                                        auto sp = std::make_shared<endpoint_t>(
                                            ioc_con, force_move(socket),
                                            version_);
                                        if (h_accept_)
                                            h_accept_(force_move(sp));
                                    });
