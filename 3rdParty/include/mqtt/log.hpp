...
template <typename T>
inline constexpr null_log const& operator<<(null_log const& o, T const&) {
    return o;
}
...
// filter and formatter can distinguish mqtt_cpp's channel and severity by their
// types
using global_logger_t =
    log::sources::severity_channel_logger<severity_level, channel>;
...
#define MQTT_LOG_FP(chan, sev)                            \
    BOOST_LOG_STREAM_CHANNEL_SEV(                         \
        MQTT_NS::logger(), MQTT_NS::channel(chan), sev)   \
...
// For example, -DMQTT_LOG_SEV=info, greater or equal to info log is generated
// at compiling time.
...
#define MQTT_LOG(chan, sev)                                       \
    BOOST_PP_IF(BOOST_PP_GREATER_EQUAL(MQTT_GET_LOG_SEV_NUM(sev), \
                    MQTT_GET_LOG_SEV_NUM(MQTT_LOG_SEV)),          \
