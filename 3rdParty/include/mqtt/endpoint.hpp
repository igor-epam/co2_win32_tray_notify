...
#if defined(MQTT_STD_ANY) && defined(__GLIBCXX__) && \
    (__GLIBCXX__ != MQTT_LIBSTDCXX_GCC_820) &&       \
    (__GLIBCXX__ != MQTT_LIBSTDCXX_GCC_830)

template <>
struct std::is_constructible<std::tuple<std::any>> : std::true_type {};

template <>
struct std::is_constructible<std::tuple<std::any>, std::tuple<std::any> const&>
    : std::true_type {};
...
template <>
struct std::is_copy_constructible<std::_Head_base<0, std::any, false>>
    : std::true_type {};

template <>
struct std::is_constructible<std::_Head_base<0, std::any, false>,
    std::_Head_base<0, std::any, false> const&> : std::true_type {};

#endif  // defined(MQTT_STD_ANY) && defined(__GLIBCXX__) && (__GLIBCXX__ !=
        // MQTT_LIBSTDCXX_GCC_820) && (__GLIBCXX__ != MQTT_LIBSTDCXX_GCC_830)
...
constexpr std::enable_if_t<
    !std::is_convertible<std::decay_t<T>, publish_options>::value, bool>
...
class endpoint : public std::enable_shared_from_this<
                     endpoint<Mutex, LockGuard, PacketIdBytes>> {
   ...
   public:
     ...
     *        - disconnect
     *          MQTT protocol requests sending connack/disconnect packet with
     * error reason code if some error happens.<BR> This function choose
     * sync/async connack/disconnect.<BR>
     */
    endpoint(as::io_context& ioc,
        ...
        MQTT_LOG("mqtt_api", info)
            << MQTT_ADD_VALUE(address, this) << "create"
            << " version:" << version << " async_operation:" << std::boolalpha
            << async_operation;
    }
     ...
     * @param ioc io_context
     * @param socket connected socket. It should have already been connected
     ...
     * sync/async connack/disconnect.<BR>
     */
    explicit endpoint(as::io_context& ioc,
   ...
   private:
     ...
     *
     */
    virtual bool on_connect(buffer client_id, optional<buffer> user_name,
        optional<buffer> password, optional<will> will, bool clean_session,
     ...
     * quit.
     */
    virtual bool on_connack(
        bool session_present, connect_return_code return_code) noexcept = 0;
     ...
     * quit.
     */
    virtual bool on_suback(packet_id_t packet_id,
        std::vector<suback_return_code> returns) noexcept = 0;
     ...
     * quit.
     */
    virtual bool on_unsubscribe(packet_id_t packet_id,
        std::vector<unsubscribe_entry> entries) noexcept = 0;
     ...
     *
     */
    virtual bool on_v5_connect(buffer client_id, optional<buffer> user_name,
        optional<buffer> password, optional<will> will, bool clean_start,
        std::uint16_t keep_alive, v5::properties props) noexcept = 0;
    ...
    virtual bool on_v5_connack(bool session_present,
        v5::connect_reason_code reason_code, v5::properties props) noexcept = 0;
    ...
    virtual bool on_v5_puback(packet_id_t packet_id,
        v5::puback_reason_code reason_code, v5::properties props) noexcept = 0;
    ...
    virtual bool on_v5_pubrec(packet_id_t packet_id,
        v5::pubrec_reason_code reason_code, v5::properties props) noexcept = 0;
    ...
    virtual bool on_v5_pubrel(packet_id_t packet_id,
        v5::pubrel_reason_code reason_code, v5::properties props) noexcept = 0;
    ...
    virtual bool on_v5_pubcomp(packet_id_t packet_id,
        v5::pubcomp_reason_code reason_code, v5::properties props) noexcept = 0;
     ...
     * quit.
     */
    virtual bool on_v5_auth(
        v5::auth_reason_code reason_code, v5::properties props) noexcept = 0;
   ...
   private:
     ...
     */
    virtual void on_serialize_publish_message(
        basic_publish_message<sizeof(packet_id_t)> msg) noexcept = 0;
     ...
     */
    virtual void on_serialize_v5_publish_message(
        v5::basic_publish_message<sizeof(packet_id_t)> msg) noexcept = 0;
     ...
     */
    virtual void on_serialize_pubrel_message(
        basic_pubrel_message<sizeof(packet_id_t)> msg) noexcept = 0;
     ...
     * @param msg pubrel message
     */
    virtual void on_serialize_v5_pubrel_message(
        v5::basic_pubrel_message<sizeof(packet_id_t)> msg) noexcept = 0;
   ...
   private:
     ...
     */
    virtual bool check_is_valid_length(control_packet_type packet_type,
        std::size_t remaining_length) noexcept = 0;
   ...
   protected:
     ...
     * finish.
     */
    MQTT_ALWAYS_INLINE virtual void on_mqtt_message_processed(
   ...
   public:
     ...
     *
     * See
     * http://docs.oasis-open.org/mqtt/mqtt/v3.1.1/os/mqtt-v3.1.1-os.html#_Toc385349231<BR>
     * 3.1.2.4 Clean Session<BR>
     * After constructing a endpoint, the clean session is set to false.
     * @return clean session
     */
    bool clean_session() const { return clean_start(); }
     ...
     */
    std::string const& get_client_id() const { return client_id_; }
     ...
     * 3.1.3.1 Client Identifier
     */
    void set_client_id(std::string id) { client_id_ = force_move(id); }
     ...
     */
    std::size_t get_total_bytes_sent() const { return total_bytes_sent_; }

    /**
     * @brief Set auto publish response mode.
     * @param b set value
     *
     * When set auto publish response mode to true, puback, pubrec, pubrel,and
     * pub comp automatically send.<BR>
     */
    void set_auto_pub_response(bool b = true) { auto_pub_response_ = b; }
     ...
     * - send store data (publish QoS1,2 and pubrel) on connack receive.
     * - disconnect
     *   MQTT protocol requests sending connack/disconnect packet with error
     * reason code if some error happens.<BR> This function choose sync/async
     * connack/disconnect.<BR>
     */
    void set_async_operation(bool async = true) { async_operation_ = async; }
    ...
    void start_session(any session_life_keeper = any()) {
        MQTT_LOG("mqtt_api", info)
            << MQTT_ADD_VALUE(address, this) << "start_session";
     ...
     *        An object that stays alive as long as the library holds a
     * reference to any other parameters. If topic_name, contents, or props do
     * not have built-in lifetime management, (e.g. buffer) use this parameter
     * to manage their lifetime.
     * @return packet_id. If qos is set to at_most_once, return 0.
     * packet_id is automatically generated.
     *
     * @note If your QOS level is exactly_once or at_least_once, then the
     * library will store this publish internally until the broker has confirmed
     * delivery, which may involve resends, and as such the life_keeper
    ...
    publish(T&& t, Params&&... params) {
        if (detail::should_generate_packet_id(params...)) {
            ...
            publish(
                packet_id, std::forward<T>(t), std::forward<Params>(params)...);
            return packet_id;
        } else {
     ...
     *        See
     * https://docs.oasis-open.org/mqtt/mqtt/v5.0/os/mqtt-v5.0-os.html#_Toc3901169<BR>
    ...
    /**
     * @brief Disconnect
     * Send a disconnect packet to the connected broker. It is a clean
     * disconnecting sequence. The broker disconnects the endpoint after
     * receives the disconnect packet.<BR> When the endpoint disconnects using
     * disconnect(), a will won't send.<BR> See
     * https://docs.oasis-open.org/mqtt/mqtt/v5.0/os/mqtt-v5.0-os.html#_Toc3901205<BR>
     * @param reason_code
     *        DISCONNECT Reason Code<BR>
     *        See
     ...
     *        3.14.2.2 DISCONNECT Properties
     */
    void disconnect(v5::disconnect_reason_code reason =
                        v5::disconnect_reason_code::normal_disconnection,
        v5::properties props = {}) {
        MQTT_LOG("mqtt_api", info)
            << MQTT_ADD_VALUE(address, this) << "disconnect"
            << " reason:" << reason;
     ...
     * When the endpoint disconnects using force_disconnect(), a will will
     * send.<BR>
     */
    void force_disconnect() {
        MQTT_LOG("mqtt_api", info)
            << MQTT_ADD_VALUE(address, this) << "force_disconnect";
     ...
     * @param packet_id
     *        packet identifier. It should be acquired by
     * acquire_unique_packet_id, or register_packet_id. The ownership of  the
     * packet_id moves to the library. If qos == qos::at_most_once, packet_id
     * must be 0. But not checked in release mode due to performance.
     * @param topic_name
     *        A topic name to publish
     * @param contents
     *        The contents to publish
     ...
     * parameter is important.
     */
    void publish(packet_id_t packet_id, std::string topic_name,
        std::string contents, publish_options pubopts = {},
        v5::properties props = {}, any life_keeper = {}) {
        MQTT_LOG("mqtt_api", info)
            << MQTT_ADD_VALUE(address, this) << "publish"
            << " pid:" << packet_id << " topic:" << topic_name
            << " qos:" << pubopts.get_qos()
            << " retain:" << pubopts.get_retain()
            << " dup:" << pubopts.get_dup();

        if (pubopts.get_qos() == qos::at_most_once) {
            // In the at_most_once case, we know a priori that send_publish
            // won't track the lifetime.
            send_publish(packet_id, as::buffer(topic_name),
                as::buffer(contents), pubopts, force_move(props), any{});
        } else {
            auto sp_topic_name =
                std::make_shared<std::string>(force_move(topic_name));
            auto sp_contents =
                std::make_shared<std::string>(force_move(contents));
            auto topic_buf = as::buffer(*sp_topic_name);
            auto contents_buf = as::buffer(*sp_contents);

            send_publish(packet_id, topic_buf, contents_buf, pubopts,
                force_move(props),
                std::make_tuple(force_move(life_keeper),
                    force_move(sp_topic_name), force_move(sp_contents)));
        }
    }
    ...
    typename std::enable_if<
        as::is_const_buffer_sequence<ConstBufferSequence>::value>::type
    publish(packet_id_t packet_id, as::const_buffer topic_name,
        ConstBufferSequence contents, publish_options pubopts,
        v5::properties props, any life_keeper) {
        MQTT_LOG("mqtt_api", info)
            << MQTT_ADD_VALUE(address, this) << "publish"
            << " pid:" << packet_id << " topic:"
            << string_view(get_pointer(topic_name), get_size(topic_name))
            << " qos:" << pubopts.get_qos()
            << " retain:" << pubopts.get_retain()
            << " dup:" << pubopts.get_dup();

        BOOST_ASSERT(
            (pubopts.get_qos() == qos::at_most_once && packet_id == 0) ||
            (pubopts.get_qos() != qos::at_most_once && packet_id != 0));

        send_publish(packet_id, topic_name, force_move(contents), pubopts,
            force_move(props), force_move(life_keeper));
    }
    ...
    publish(packet_id_t packet_id, as::const_buffer topic_name,
        ...
        send_publish(packet_id, topic_name, force_move(contents), pubopts,
            v5::properties{}, force_move(life_keeper));
    }
     ...
     * library will store this publish internally until the broker has confirmed
     * delivery, which may involve resends, and as such the life_keeper
     * parameter is important. If topic_name and contents don't manage their
     * lifetimes, then life_keeper should be used to keep their lifetimes.
     */
    template <typename BufferSequence>
    typename std::enable_if<is_buffer_sequence<BufferSequence>::value>::type
    publish(packet_id_t packet_id, buffer topic_name, BufferSequence contents,
        publish_options pubopts = {}, any life_keeper = {}) {
        MQTT_LOG("mqtt_api", info)
            << MQTT_ADD_VALUE(address, this) << "publish"
            << " pid:" << packet_id << " topic:" << topic_name
            << " qos:" << pubopts.get_qos()
        ...
        send_publish(packet_id, topic_name_buf, force_move(cbs), pubopts,
            v5::properties{},
            std::make_tuple(force_move(life_keeper), force_move(topic_name),
                force_move(contents)));
    }
    ...
    typename std::enable_if<is_buffer_sequence<BufferSequence>::value>::type
    publish(packet_id_t packet_id, buffer topic_name, BufferSequence contents,
        publish_options pubopts, v5::properties props, any life_keeper = {}) {
        MQTT_LOG("mqtt_api", info)
            << MQTT_ADD_VALUE(address, this) << "publish"
            << " pid:" << packet_id << " topic:" << topic_name
            << " qos:" << pubopts.get_qos()
     ...
     *        packet identifier. It should be acquired by
     * acquire_unique_packet_id, or register_packet_id. The ownership of  the
     * packet_id moves to the library.
     * @param topic_filter
     ...
     * You can subscribe multiple topics all at once.<BR>
     * See
     * https://docs.oasis-open.org/mqtt/mqtt/v5.0/os/mqtt-v5.0-os.html#_Toc3901161<BR>
     */
    void subscribe(packet_id_t packet_id, string_view topic_filter,
        subscribe_options option, v5::properties props = {}) {
        MQTT_LOG("mqtt_api", info)
            << MQTT_ADD_VALUE(address, this) << "subscribe"
            << " pid:" << packet_id << " topic:" << topic_filter
            << " qos:" << option.get_qos()
            << " rh:" << option.get_retain_handling()
            << " nl:" << option.get_nl() << " rap:" << option.get_rap();

        send_subscribe(
            std::vector<std::tuple<as::const_buffer, subscribe_options>>{
                {as::buffer(topic_filter.data(), topic_filter.size()), option}},
            packet_id, force_move(props));
    }
     ...
     * https://docs.oasis-open.org/mqtt/mqtt/v5.0/os/mqtt-v5.0-os.html#_Toc3901161<BR>
     */
    void subscribe(packet_id_t packet_id, as::const_buffer topic_filter,
        subscribe_options option, v5::properties props = {}) {
        MQTT_LOG("mqtt_api", info)
            << MQTT_ADD_VALUE(address, this) << "subscribe"
            << " pid:" << packet_id << " topic:"
            << string_view(get_pointer(topic_filter), get_size(topic_filter))
            << " qos:" << option.get_qos()
            << " rh:" << option.get_retain_handling()
            << " nl:" << option.get_nl() << " rap:" << option.get_rap();
     ...
     * https://docs.oasis-open.org/mqtt/mqtt/v5.0/os/mqtt-v5.0-os.html#_Toc3901161<BR>
     */
    void subscribe(packet_id_t packet_id,
        ...
        for (auto const& e : params) {
            cb_params.emplace_back(
                as::buffer(std::get<0>(e).data(), std::get<0>(e).size()),
                std::get<1>(e));
        }
     ...
     * https://docs.oasis-open.org/mqtt/mqtt/v5.0/os/mqtt-v5.0-os.html#_Toc3901179
     */
    void unsubscribe(packet_id_t packet_id, string_view topic_filter,
        v5::properties props = {}) {
        MQTT_LOG("mqtt_api", info)
            << MQTT_ADD_VALUE(address, this) << "unsubscribe"
            << " pid:" << packet_id << " topic:" << topic_filter;

        send_unsubscribe(std::vector<as::const_buffer>{as::buffer(
     ...
     * https://docs.oasis-open.org/mqtt/mqtt/v5.0/os/mqtt-v5.0-os.html#_Toc3901179
     */
    void unsubscribe(packet_id_t packet_id, as::const_buffer topic_filter,
        v5::properties props = {}) {
        MQTT_LOG("mqtt_api", info)
            << MQTT_ADD_VALUE(address, this) << "unsubscribe"
            << " pid:" << packet_id << " topic:"
        ...
        send_unsubscribe(std::vector<as::const_buffer>{topic_filter}, packet_id,
            force_move(props));
    }
     ...
     * https://docs.oasis-open.org/mqtt/mqtt/v5.0/os/mqtt-v5.0-os.html#_Toc3901179
     */
    void unsubscribe(packet_id_t packet_id, std::vector<string_view> params,
        v5::properties props = {}) {
        MQTT_LOG("mqtt_api", info)
            << MQTT_ADD_VALUE(address, this) << "unsubscribe"
            << " pid:" << packet_id;
     ...
     * https://docs.oasis-open.org/mqtt/mqtt/v5.0/os/mqtt-v5.0-os.html#_Toc3901179
     */
    void unsubscribe(packet_id_t packet_id,
        std::vector<as::const_buffer> params, v5::properties props = {}) {
        MQTT_LOG("mqtt_api", info)
            << MQTT_ADD_VALUE(address, this) << "unsubscribe"
            << " pid:" << packet_id;
        ...
        for (auto&& e : params) {
            cb_params.emplace_back(
                buffer(string_view(get_pointer(e), get_size(e))));
        }
     ...
     * https://docs.oasis-open.org/mqtt/mqtt/v5.0/os/mqtt-v5.0-os.html#_Toc3901179
     */
    void unsubscribe(packet_id_t packet_id, std::vector<buffer> params,
        v5::properties props = {}) {
        MQTT_LOG("mqtt_api", info)
            << MQTT_ADD_VALUE(address, this) << "unsubscribe"
            << " pid:" << packet_id;
    ...
    void pingreq() {
        ...
        MQTT_LOG("mqtt_api", trace)
            << MQTT_ADD_VALUE(address, this) << "pingreq";
    ...
    void pingresp() {
        ...
        MQTT_LOG("mqtt_api", trace)
            << MQTT_ADD_VALUE(address, this) << "pingrsp";
     ...
     * See
     * https://docs.oasis-open.org/mqtt/mqtt/v5.0/os/mqtt-v5.0-os.html#_Toc398718086
     * @param reason_code
     *        AUTH Reason Code<BR>
     *        See
     * https://docs.oasis-open.org/mqtt/mqtt/v5.0/os/mqtt-v5.0-os.html#_Toc3901220<BR>
     ...
     *        3.15.2.2 AUTH Properties
     */
    void auth(v5::auth_reason_code reason_code = v5::auth_reason_code::success,
        v5::properties props = {}) {
        MQTT_LOG("mqtt_api", info) << MQTT_ADD_VALUE(address, this) << "auth"
                                   << " reason:" << reason_code;
     ...
     * @param w
     *        See
     * https://docs.oasis-open.org/mqtt/mqtt/v5.0/os/mqtt-v5.0-os.html#_Toc479576982<BR>
     *        3.1.2.5 Will Flag
     * @param clean_session
     *        See
     * https://docs.oasis-open.org/mqtt/mqtt/v5.0/os/mqtt-v5.0-os.html#_Toc3901039<BR>
     ...
     *        See
     * https://docs.oasis-open.org/mqtt/mqtt/v5.0/os/mqtt-v5.0-os.html#_Toc385349238
     *        See
     * https://docs.oasis-open.org/mqtt/mqtt/v5.0/os/mqtt-v5.0-os.html#_Toc398718028
     * @param keep_alive_sec
     *        Keep Alive<BR>
     *        See
     * https://docs.oasis-open.org/mqtt/mqtt/v5.0/os/mqtt-v5.0-os.html#_Toc3901045<BR>
     ...
     *        3.1.2.11 CONNECT Properties
     */
    void connect(std::string const& client_id,
        optional<std::string> const& user_name,
        optional<std::string> const& password, optional<will> w,
        std::uint16_t keep_alive_sec, v5::properties props = {}) {
        MQTT_LOG("mqtt_api", info)
            << MQTT_ADD_VALUE(address, this) << "connect"
            << " client_id:" << client_id
        ...
        send_connect(
            ...
            force_move(w), keep_alive_sec, force_move(props));
    }
     ...
     *        3.1.2.11 CONNECT Properties
     */
    void connect(buffer client_id, optional<buffer> user_name,
        optional<buffer> password, optional<will> w,
        std::uint16_t keep_alive_sec, v5::properties props = {}) {
        MQTT_LOG("mqtt_api", info)
            << MQTT_ADD_VALUE(address, this) << "connect"
            << " client_id:" << client_id << " user_name:"
            << (user_name ? string_view(user_name.value())
                          : string_view("none"))
            << " keep_alive:" << std::dec << keep_alive_sec;

        connect_requested_ = true;
        send_connect(force_move(client_id), force_move(user_name),
     ...
     * @param session_present See
     * https://docs.oasis-open.org/mqtt/mqtt/v5.0/os/mqtt-v5.0-os.html#_Toc385349255
     * @param reason_code See reason_code.hpp and
     * https://docs.oasis-open.org/mqtt/mqtt/v5.0/os/mqtt-v5.0-os.html#_Toc385349256
     * @param props
     *        Properties<BR>
     *        See
     * https://docs.oasis-open.org/mqtt/mqtt/v5.0/os/mqtt-v5.0-os.html#_Toc3901080<BR>
     *        3.2.2.3 CONNACK Properties
     * See
     * https://docs.oasis-open.org/mqtt/mqtt/v5.0/os/mqtt-v5.0-os.html#_Toc398718033
     */
    void connack(bool session_present,
        variant<connect_return_code, v5::connect_reason_code> reason_code,
        v5::properties props = {}) {
        MQTT_LOG("mqtt_api", info)
            << MQTT_ADD_VALUE(address, this) << "connack"
    ...
    void puback(packet_id_t packet_id,
        ...
        v5::properties props = {}) {
        MQTT_LOG("mqtt_api", info)
            << MQTT_ADD_VALUE(address, this) << "puback"
            << " pid:" << packet_id << " reason:" << reason_code;
    ...
    void pubrec(packet_id_t packet_id,
        ...
        v5::properties props = {}) {
        MQTT_LOG("mqtt_api", info)
            << MQTT_ADD_VALUE(address, this) << "pubrec"
            << " pid:" << packet_id << " reason:" << reason_code;
     ...
     * @param life_keeper
     *        An object that stays alive as long as the library holds a
     * reference to any other parameters. If your props are not using built-in
     * lifetime management, (e.g. buffer) use this parameter to manage their
     * lifetime.
     *
     * @note The library may store this message while it communicates with the
     * server for several round trips. As such, the life_keeper paramter is
     * important.
     */
    void pubrel(packet_id_t packet_id,
        ...
        v5::properties props = {}, any life_keeper = {}) {
        MQTT_LOG("mqtt_api", info)
            << MQTT_ADD_VALUE(address, this) << "pubrel"
            << " pid:" << packet_id << " reason:" << reason_code;

        send_pubrel(
            packet_id, reason_code, force_move(props), force_move(life_keeper));
    }
    ...
    void pubcomp(packet_id_t packet_id,
        ...
        v5::properties props = {}) {
        MQTT_LOG("mqtt_api", info)
            << MQTT_ADD_VALUE(address, this) << "pubcomp"
            << " pid:" << packet_id << " reason:" << reason_code;
     ...
     *        3.9.2.1 SUBACK Properties
     */
    void suback(packet_id_t packet_id,
        variant<suback_return_code, v5::suback_reason_code> reason,
        v5::properties props = {}) {
        MQTT_LOG("mqtt_api", info) << MQTT_ADD_VALUE(address, this) << "suback"
                                   << " pid:" << packet_id << " reason:" <
            reason;

        if (variant_idx(reason) == 0) {
            send_suback(
                std::vector<suback_return_code>{
                    variant_get<suback_return_code>(reason)},
                packet_id, force_move(props));
        } else {
            send_suback(
                std::vector<v5::suback_reason_code>{
                    variant_get<v5::suback_reason_code>(reason)},
                packet_id, force_move(props));
        }
    }
     ...
     */
    void suback(packet_id_t packet_id,
        variant<std::vector<suback_return_code>,
            std::vector<v5::suback_reason_code>>
            reasons,
        v5::properties props = {}) {
        MQTT_LOG("mqtt_api", info) << MQTT_ADD_VALUE(address, this) << "suback"
    ...
    void unsuback(packet_id_t packet_id) {
        MQTT_LOG("mqtt_api", info)
            << MQTT_ADD_VALUE(address, this) << "unsuback"
            << " pid:" << packet_id;
     ...
     *        3.11.2.1 UNSUBACK Properties
     */
    void unsuback(packet_id_t packet_id, v5::unsuback_reason_code reason,
        v5::properties props = {}) {
        MQTT_LOG("mqtt_api", info)
            << MQTT_ADD_VALUE(address, this) << "unsuback"
            << " pid:" << packet_id << " reason:" << reason;

        send_unsuback(std::vector<v5::unsuback_reason_code>{reason}, packet_id,
            force_move(props));
    }
    ...
    async_publish(T&& t, Params&&... params) {
        if (detail::should_generate_packet_id(params...)) {
            ...
            async_publish(
                0, std::forward<T>(t), std::forward<Params>(params)...);
        }
    }
     ...
     * @brief Disconnect
     * @param func
     *        functor object who's operator() will be called when the async
     * operation completes. Send a disconnect packet to the connected broker. It
     * is a clean disconnecting sequence. The broker disconnects the endpoint
     * after receives the disconnect packet.<BR> When the endpoint disconnects
     * using disconnect(), a will won't send.<BR>
     */
    void async_disconnect(async_handler_t func = {}) {
        MQTT_LOG("mqtt_api", info)
            << MQTT_ADD_VALUE(address, this) << "async_disconnect";

        if (connected_ && mqtt_connected_) {
            disconnect_requested_ = true;
            // The reason code and property vector are only used if we're using
            // mqttv5.
            async_send_disconnect(
                v5::disconnect_reason_code::normal_disconnection,
                v5::properties{}, force_move(func));
        } else {
            socket_->post([func = force_move(func)] {
                if (func)
                    func(boost::system::errc::make_error_code(
     ...
     * using disconnect(), a will won't send.<BR>
     */
    void async_disconnect(v5::disconnect_reason_code reason,
        v5::properties props = {}, async_handler_t func = {}) {
        MQTT_LOG("mqtt_api", info)
            << MQTT_ADD_VALUE(address, this) << "async_disconnect"
            << " reason:" << reason;
     ...
     * @param func
     *        functor object who's operator() will be called when the async
     * operation completes. Force disconnect. It is not a clean disconnect
     * sequence.<BR> When the endpoint disconnects using force_disconnect(), a
     * will will send.<BR>
     */
    void async_force_disconnect(async_handler_t func = {}) {
        MQTT_LOG("mqtt_api", info)
            << MQTT_ADD_VALUE(address, this) << "async_force_disconnect";
        socket_->post([this, self = this->shared_from_this(),
     ...
     * @param func
     *        functor object who's operator() will be called when the async
     * operation completes. packet_id is automatically generated.<BR> You can
     * subscribe multiple topics all at once.<BR> See
     * https://docs.oasis-open.org/mqtt/mqtt/v5.0/os/mqtt-v5.0-os.html#_Toc3901161
     */
    template <typename T, typename... Params>
    std::enable_if_t<!std::is_convertible<std::decay_t<T>, packet_id_t>::value>
     ...
     * operation completes.
     */
    void async_publish(packet_id_t packet_id, std::string topic_name,
        std::string contents, publish_options pubopts = {},
        async_handler_t func = {}) {
        MQTT_LOG("mqtt_api", info)
            << MQTT_ADD_VALUE(address, this) << "async_publish"
            << " pid:" << packet_id << " topic:" << topic_name
            << " qos:" << pubopts.get_qos()
            << " retain:" << pubopts.get_retain()
            << " dup:" << pubopts.get_dup();
        ...
        async_send_publish(packet_id, topic_name_buf, contents_buf, pubopts,
            v5::properties{},
     ...
     */
    void async_publish(packet_id_t packet_id, std::string topic_name,
        std::string contents, publish_options pubopts, v5::properties props,
        any life_keeper = {}, async_handler_t func = {}) {
        MQTT_LOG("mqtt_api", info)
            << MQTT_ADD_VALUE(address, this) << "async_publish"
            << " pid:" << packet_id << " topic:" << topic_name
            << " qos:" << pubopts.get_qos()
        ...
        async_send_publish(packet_id, topic_name_buf, contents_buf, pubopts,
            force_move(props),
            std::make_tuple(force_move(life_keeper), force_move(sp_topic_name),
    ...
    typename std::enable_if<
        as::is_const_buffer_sequence<ConstBufferSequence>::value>::type
    async_publish(packet_id_t packet_id, as::const_buffer topic_name,
        ConstBufferSequence contents, publish_options pubopts = {},
        any life_keeper = {}, async_handler_t func = {}) {
        MQTT_LOG("mqtt_api", info)
            << MQTT_ADD_VALUE(address, this) << "async_publish"
        ...
        async_send_publish(packet_id, topic_name, force_move(contents), pubopts,
            v5::properties{}, force_move(life_keeper), force_move(func));
    }
    ...
    async_publish(packet_id_t packet_id, as::const_buffer topic_name,
        ConstBufferSequence contents, publish_options pubopts,
        v5::properties props, any life_keeper = {}, async_handler_t func = {}) {
        MQTT_LOG("mqtt_api", info)
            << MQTT_ADD_VALUE(address, this) << "async_publish"
            << " pid:" << packet_id << " topic:"
            << string_view(get_pointer(topic_name), get_size(topic_name))
        ...
        async_send_publish(packet_id, topic_name, force_move(contents), pubopts,
            force_move(props), force_move(life_keeper), force_move(func));
    }
    ...
    template <typename BufferSequence>
    typename std::enable_if<is_buffer_sequence<BufferSequence>::value>::type
    async_publish(packet_id_t packet_id, buffer topic_name,
        BufferSequence contents, publish_options pubopts = {},
        any life_keeper = {}, async_handler_t func = {}) {
        MQTT_LOG("mqtt_api", info)
            << MQTT_ADD_VALUE(address, this) << "async_publish"
            << " pid:" << packet_id << " topic:" << topic_name
        ...
        async_send_publish(packet_id, topic_name_buf, force_move(cbs), pubopts,
            v5::properties{},
            std::make_tuple(force_move(life_keeper), force_move(topic_name),
                force_move(contents)),
            force_move(func));
    ...
    typename std::enable_if<is_buffer_sequence<BufferSequence>::value>::type
    async_publish(packet_id_t packet_id, buffer topic_name,
        BufferSequence contents, publish_options pubopts, v5::properties props,
        any life_keeper = {}, async_handler_t func = {}) {
        MQTT_LOG("mqtt_api", info)
            << MQTT_ADD_VALUE(address, this) << "async_publish"
            << " pid:" << packet_id << " topic:" << topic_name
     ...
     * @param func
     *        functor object who's operator() will be called when the async
     * operation completes. You can subscribe multiple topics all at once.<BR>
     * See
     * https://docs.oasis-open.org/mqtt/mqtt/v5.0/os/mqtt-v5.0-os.html#_Toc3901161
     */
    void async_subscribe(packet_id_t packet_id, std::string topic_filter,
        subscribe_options option, async_handler_t func = {}) {
        MQTT_LOG("mqtt_api", info)
            << MQTT_ADD_VALUE(address, this) << "async_subscribe"
            << " pid:" << packet_id << " topic:" << topic_filter
            << " qos:" << option.get_qos()
            << " rh:" << option.get_retain_handling()
            << " nl:" << option.get_nl() << " rap:" << option.get_rap();

        auto sp_topic_filter =
            std::make_shared<std::string>(force_move(topic_filter));
        auto topic_filter_buf = as::buffer(*sp_topic_filter);

        async_send_subscribe(
            std::vector<std::tuple<as::const_buffer, subscribe_options>>{
                {topic_filter_buf, option}},
            packet_id, v5::properties{},
            [life_keeper = force_move(sp_topic_filter),
                func = force_move(func)](error_code ec) {
                if (func) func(ec);
            });
    }
     ...
     * @param func
     *        functor object who's operator() will be called when the async
     * operation completes. This object should hold the lifetime of the buffers
     * for topic_filter. You can subscribe multiple topics all at once.<BR> See
     * https://docs.oasis-open.org/mqtt/mqtt/v5.0/os/mqtt-v5.0-os.html#_Toc3901161
     */
    void async_subscribe(packet_id_t packet_id, as::const_buffer topic_filter,
        subscribe_options option, async_handler_t func) {
        MQTT_LOG("mqtt_api", info)
            << MQTT_ADD_VALUE(address, this) << "async_subscribe"
            << " pid:" << packet_id << " topic:"
        ...
        async_send_subscribe(
            std::vector<std::tuple<as::const_buffer, subscribe_options>>{
                {topic_filter, option}},
            packet_id, v5::properties{}, force_move(func));
    }
     ...
     *        functor object who's operator() will be called when the async
     * operation completes. This object should hold the lifetime of the buffers
     * for topic_filter, and properties. You can subscribe multiple topics all
     * at once.<BR> See
     * https://docs.oasis-open.org/mqtt/mqtt/v5.0/os/mqtt-v5.0-os.html#_Toc3901161
     */
    void async_subscribe(packet_id_t packet_id, as::const_buffer topic_filter,
        subscribe_options option, v5::properties props, async_handler_t func) {
        MQTT_LOG("mqtt_api", info)
            << MQTT_ADD_VALUE(address, this) << "async_subscribe"
            << " pid:" << packet_id << " topic:"
            << string_view(get_pointer(topic_filter), get_size(topic_filter))
        ...
        async_send_subscribe(
            std::vector<std::tuple<as::const_buffer, subscribe_options>>{
                {topic_filter, option}},
            packet_id, force_move(props), force_move(func));
    }
     ...
     * https://docs.oasis-open.org/mqtt/mqtt/v5.0/os/mqtt-v5.0-os.html#_Toc3901161
     */
    void async_subscribe(packet_id_t packet_id, buffer topic_filter,
        subscribe_options option, async_handler_t func = {}) {
        MQTT_LOG("mqtt_api", info)
            << MQTT_ADD_VALUE(address, this) << "async_subscribe"
            << " pid:" << packet_id << " topic:" << topic_filter
     ...
     * packet_id moves to the library.
     * @param params
     *        A collection of the pair of topic_filter and option to
     * subscribe.<BR> See
     * https://docs.oasis-open.org/mqtt/mqtt/v5.0/os/mqtt-v5.0-os.html#_Toc3901169<BR>
     *        3.8.3.1 Subscription Options
     * @param func
     ...
     * https://docs.oasis-open.org/mqtt/mqtt/v5.0/os/mqtt-v5.0-os.html#_Toc3901161
     */
    void async_subscribe(packet_id_t packet_id,
        ...
        for (auto&& e : params) {
            auto sp_topic_filter =
                std::make_shared<std::string>(force_move(std::get<0>(e)));
            cb_params.emplace_back(
                as::buffer(*sp_topic_filter), std::get<1>(e));
            life_keepers.emplace_back(force_move(sp_topic_filter));
        }

        async_send_subscribe(force_move(cb_params), packet_id, v5::properties{},
    ...
    void async_subscribe(packet_id_t packet_id,
        ...
        async_send_subscribe(force_move(cb_params), packet_id,
            force_move(props),
            [life_keeper = force_move(life_keepers), func = force_move(func)](
                error_code ec) {
                if (func) func(ec);
     ...
     * acquire_unique_packet_id, or register_packet_id. The ownership of  the
     * packet_id moves to the library.
     * @param params A collection of the pair of topic_filter and qos to
     * subscribe.
     * @param func
     *        functor object who's operator() will be called when the async
     * operation completes. This object should hold the lifetime of the buffers
     * for params. You can subscribe multiple topics all at once.<BR> See
     * https://docs.oasis-open.org/mqtt/mqtt/v5.0/os/mqtt-v5.0-os.html#_Toc3901161
     */
    void async_subscribe(packet_id_t packet_id,
        ...
        async_send_subscribe(
            force_move(params), packet_id, v5::properties{}, force_move(func));
    }
    ...
    void async_subscribe(packet_id_t packet_id,
        ...
        async_send_subscribe(
            force_move(params), packet_id, force_move(props), force_move(func));
    }
     ...
     * https://docs.oasis-open.org/mqtt/mqtt/v5.0/os/mqtt-v5.0-os.html#_Toc3901161
     */
    void async_unsubscribe(packet_id_t packet_id, std::string topic_filter,
        async_handler_t func = {}) {
        MQTT_LOG("mqtt_api", info)
            << MQTT_ADD_VALUE(address, this) << "async_unsubscribe"
            << " pid:" << packet_id << " topic:" << topic_filter;
     ...
     * @param func
     *        functor object who's operator() will be called when the async
     * operation completes. This object should hold the lifetime of the buffer
     * for topic_filter. You can subscribe multiple topics all at once.<BR> See
     * https://docs.oasis-open.org/mqtt/mqtt/v5.0/os/mqtt-v5.0-os.html#_Toc3901161
     */
    void async_unsubscribe(packet_id_t packet_id, as::const_buffer topic_filter,
        async_handler_t func) {
        MQTT_LOG("mqtt_api", info)
            << MQTT_ADD_VALUE(address, this) << "async_unsubscribe"
            << " pid:" << packet_id << " topic:"
        ...
        async_send_unsubscribe(std::vector<as::const_buffer>{topic_filter},
            packet_id, v5::properties{}, force_move(func));
    }
    ...
    void async_unsubscribe(
        packet_id_t packet_id, buffer topic_filter, async_handler_t func = {}) {
        MQTT_LOG("mqtt_api", info)
            << MQTT_ADD_VALUE(address, this) << "async_unsubscribe"
            << " pid:" << packet_id << " topic:" << topic_filter;
     ...
     * https://docs.oasis-open.org/mqtt/mqtt/v5.0/os/mqtt-v5.0-os.html#_Toc3901161
     */
    void async_unsubscribe(packet_id_t packet_id, buffer topic_filter,
        v5::properties props, async_handler_t func = {}) {
        MQTT_LOG("mqtt_api", info)
            << MQTT_ADD_VALUE(address, this) << "async_unsubscribe"
            << " pid:" << packet_id << " topic:" << topic_filter;
     ...
     * https://docs.oasis-open.org/mqtt/mqtt/v5.0/os/mqtt-v5.0-os.html#_Toc3901179
     */
    void async_unsubscribe(packet_id_t packet_id,
        std::vector<std::string> params, async_handler_t func = {}) {
        MQTT_LOG("mqtt_api", info)
            << MQTT_ADD_VALUE(address, this) << "async_unsubscribe"
            << " pid:" << packet_id;
        ...
        for (auto&& e : params) {
            life_keepers.emplace_back(
                std::make_shared<std::string>(force_move(e)));
            cb_params.emplace_back(as::buffer(*life_keepers.back()));
        }

        async_send_unsubscribe(force_move(cb_params), packet_id,
            v5::properties{},
            [life_keeper = force_move(life_keepers), func = force_move(func)](
                error_code ec) {
                if (func) func(ec);
     ...
     */
    void async_unsubscribe(packet_id_t packet_id,
        std::vector<std::string> params, v5::properties props,
        async_handler_t func = {}) {
        MQTT_LOG("mqtt_api", info)
            << MQTT_ADD_VALUE(address, this) << "async_unsubscribe"
            << " pid:" << packet_id;
        ...
        // TODO: Does vector do short-buffer-optimization? If so, this is an
        // invalid idea.
        std::vector<std::shared_ptr<std::string>> life_keepers;
        life_keepers.reserve(params.size());
     ...
     * @param func
     *        functor object who's operator() will be called when the async
     * operation completes. This object may hold the lifetime of the buffers for
     * topic_filter and contents. You can subscribe multiple topics all at
     * once.<BR> See
     * https://docs.oasis-open.org/mqtt/mqtt/v5.0/os/mqtt-v5.0-os.html#_Toc3901179
     */
    void async_unsubscribe(packet_id_t packet_id,
        std::vector<as::const_buffer> params, async_handler_t func) {
        MQTT_LOG("mqtt_api", info)
            << MQTT_ADD_VALUE(address, this) << "async_unsubscribe"
            << " pid:" << packet_id;
     ...
     * https://docs.oasis-open.org/mqtt/mqtt/v5.0/os/mqtt-v5.0-os.html#_Toc3901179
     */
    void async_unsubscribe(packet_id_t packet_id, std::vector<buffer> params,
        async_handler_t func = {}) {
        MQTT_LOG("mqtt_api", info)
            << MQTT_ADD_VALUE(address, this) << "async_unsubscribe"
            << " pid:" << packet_id;
    ...
    void async_pingreq(async_handler_t func = {}) {
        MQTT_LOG("mqtt_api", trace)
            << MQTT_ADD_VALUE(address, this) << "async_pingreq";
    ...
    void async_pingresp(async_handler_t func = {}) {
        MQTT_LOG("mqtt_api", trace)
            << MQTT_ADD_VALUE(address, this) << "async_pingrsp";
    ...
    void async_auth(
        ...
        v5::properties props = {}, async_handler_t func = {}) {
        MQTT_LOG("mqtt_api", info)
            << MQTT_ADD_VALUE(address, this) << "async_auth"
            << " reason:" << reason_code;
     ...
     * https://docs.oasis-open.org/mqtt/mqtt/v5.0/os/mqtt-v5.0-os.html#_Toc398718028
     */
    void async_connect(buffer client_id, optional<buffer> user_name,
        optional<buffer> password, optional<will> w,
        std::uint16_t keep_alive_sec, async_handler_t func = {}) {
        async_connect(force_move(client_id), force_move(user_name),
            force_move(password), force_move(w), keep_alive_sec,
            v5::properties{}, force_move(func));
    }
    ...
    void async_connect(buffer client_id, optional<buffer> user_name,
        ...
        async_handler_t func = {}) {
        MQTT_LOG("mqtt_api", info)
            << MQTT_ADD_VALUE(address, this) << "async_connect"
            << " client_id:" << client_id << " user_name:"
            << (user_name ? string_view(user_name.value())
                          : string_view("none"))
            << " keep_alive:" << std::dec << keep_alive_sec;
        ...
        connect_requested_ = true;
        async_send_connect(force_move(client_id), force_move(user_name),
            force_move(password), force_move(w), keep_alive_sec,
            force_move(props), force_move(func));
    }
     ...
     * @param session_present See
     * https://docs.oasis-open.org/mqtt/mqtt/v5.0/os/mqtt-v5.0-os.html#_Toc385349255
     * @param return_code See connect_return_code.hpp and
     * https://docs.oasis-open.org/mqtt/mqtt/v5.0/os/mqtt-v5.0-os.html#_Toc385349256
     * @param func
     *        functor object who's operator() will be called when the async
     * operation completes. See
     * https://docs.oasis-open.org/mqtt/mqtt/v5.0/os/mqtt-v5.0-os.html#_Toc398718033
     */
    void async_connack(bool session_present,
        variant<connect_return_code, v5::connect_reason_code> reason_code,
        async_handler_t func = {}) {
        MQTT_LOG("mqtt_api", info)
            << MQTT_ADD_VALUE(address, this) << "async_connack"
            << " session_present:" << std::boolalpha << session_present
            << " reason:" << reason_code;

        async_send_connack(session_present, force_move(reason_code),
            v5::properties{}, force_move(func));
    }
     ...
     *        functor object who's operator() will be called when the async
     * operation completes. See
     * https://docs.oasis-open.org/mqtt/mqtt/v5.0/os/mqtt-v5.0-os.html#_Toc398718043
     */
    void async_puback(packet_id_t packet_id, async_handler_t func = {}) {
        MQTT_LOG("mqtt_api", info)
            << MQTT_ADD_VALUE(address, this) << "async_puback"
            << " pid:" << packet_id;

        async_send_puback(packet_id, v5::puback_reason_code::success,
            v5::properties{}, force_move(func));
    }
     ...
     * https://docs.oasis-open.org/mqtt/mqtt/v5.0/os/mqtt-v5.0-os.html#_Toc398718043
     */
    void async_puback(packet_id_t packet_id, v5::puback_reason_code reason_code,
        v5::properties props, async_handler_t func = {}) {
        MQTT_LOG("mqtt_api", info)
            << MQTT_ADD_VALUE(address, this) << "async_puback"
            << " pid:" << packet_id << " reason:" << reason_code;
        ...
        async_send_puback(
            packet_id, reason_code, force_move(props), force_move(func));
    }
    ...
    void async_pubrec(packet_id_t packet_id, async_handler_t func = {}) {
        MQTT_LOG("mqtt_api", info)
            << MQTT_ADD_VALUE(address, this) << "async_pubrec"
            << " pid:" << packet_id;

        async_send_pubrec(packet_id, v5::pubrec_reason_code::success,
            v5::properties{}, force_move(func));
    }
     ...
     * https://docs.oasis-open.org/mqtt/mqtt/v5.0/os/mqtt-v5.0-os.html#_Toc398718043
     */
    void async_pubrec(packet_id_t packet_id, v5::pubrec_reason_code reason_code,
        v5::properties props, async_handler_t func = {}) {
        MQTT_LOG("mqtt_api", info)
            << MQTT_ADD_VALUE(address, this) << "async_pubrec"
            << " pid:" << packet_id << " reason:" << reason_code;
    ...
    void async_pubrel(packet_id_t packet_id, async_handler_t func = {}) {
        MQTT_LOG("mqtt_api", info)
            << MQTT_ADD_VALUE(address, this) << "async_pubrel"
            << " pid:" << packet_id;

        async_send_pubrel(packet_id, v5::pubrel_reason_code::success,
            v5::properties{}, any(), force_move(func));
    }
    ...
    std::enable_if_t<std::is_convertible<Func, async_handler_t>::value>
    async_pubrel(packet_id_t packet_id, v5::pubrel_reason_code reason_code,
        v5::properties props = {}, Func&& func = {}) {
        MQTT_LOG("mqtt_api", info)
            << MQTT_ADD_VALUE(address, this) << "async_pubrel"
            << " pid:" << packet_id << " reason:" << reason_code;

        async_send_pubrel(packet_id, reason_code, force_move(props), any(),
     ...
     * important.
     */
    void async_pubrel(packet_id_t packet_id, v5::pubrel_reason_code reason_code,
        v5::properties props = {}, any life_keeper = {},
        async_handler_t func = {}) {
        MQTT_LOG("mqtt_api", info)
            << MQTT_ADD_VALUE(address, this) << "async_pubrel"
        ...
        async_send_pubrel(packet_id, reason_code, force_move(props),
            force_move(life_keeper), force_move(func));
    }
    ...
    void async_pubcomp(packet_id_t packet_id, async_handler_t func = {}) {
        MQTT_LOG("mqtt_api", info)
            << MQTT_ADD_VALUE(address, this) << "async_pubcomp"
            << " pid:" << packet_id;

        async_send_pubcomp(packet_id, v5::pubcomp_reason_code::success,
            v5::properties{}, force_move(func));
    }
     ...
     * https://docs.oasis-open.org/mqtt/mqtt/v5.0/os/mqtt-v5.0-os.html#_Toc398718043
     */
    void async_pubcomp(packet_id_t packet_id,
     ...
     *        functor object who's operator() will be called when the async
     * operation completes. See
     * https://docs.oasis-open.org/mqtt/mqtt/v5.0/os/mqtt-v5.0-os.html#_Toc398718068
     */
    void async_suback(packet_id_t packet_id,
        variant<suback_return_code, v5::suback_reason_code> reason,
        async_handler_t func = {}) {
        MQTT_LOG("mqtt_api", info)
                << MQTT_ADD_VALUE(address, this) << "async_suback"
                << " pid:" << packet_id << " reason:" <
            reason;
    ...
    void async_suback(packet_id_t packet_id,
        ...
        async_send_suback(
            force_move(reasons), packet_id, v5::properties{}, force_move(func));
    }
    ...
    void async_suback(packet_id_t packet_id,
        ...
        async_send_suback(force_move(reasons), packet_id, force_move(props),
            force_move(func));
    }
     ...
     * https://docs.oasis-open.org/mqtt/mqtt/v5.0/os/mqtt-v5.0-os.html#_Toc398718068
     */
    void async_unsuback(packet_id_t packet_id, v5::unsuback_reason_code reason,
        async_handler_t func = {}) {
        MQTT_LOG("mqtt_api", info)
                << MQTT_ADD_VALUE(address, this) << "async_unsuback"
                << " pid:" << packet_id << " reason:" <
            reason;

        async_send_unsuback(std::vector<v5::unsuback_reason_code>{reason},
            packet_id, force_move(func));
    }
     ...
     * https://docs.oasis-open.org/mqtt/mqtt/v5.0/os/mqtt-v5.0-os.html#_Toc398718068
     */
    void async_unsuback(packet_id_t packet_id,
        std::vector<v5::unsuback_reason_code> reasons,
        async_handler_t func = {}) {
        MQTT_LOG("mqtt_api", info)
            << MQTT_ADD_VALUE(address, this) << "async_unsuback"
    ...
    void async_unsuback(packet_id_t packet_id,
        ...
        async_send_unsuback(force_move(reasons), packet_id, force_move(props),
            force_move(func));
    }
     ...
     *        functor object who's operator() will be called when the async
     * operation completes. See
     * https://docs.oasis-open.org/mqtt/mqtt/v5.0/os/mqtt-v5.0-os.html#_Toc398718077
     */
     ...
     */
    void for_each_store(
        std::function<void(char const*, std::size_t)> const& f) {
        MQTT_LOG("mqtt_api", info)
            << MQTT_ADD_VALUE(address, this) << "for_each_store(ptr, size)";
        LockGuard<Mutex> lck(store_mtx_);
     ...
     */
    void for_each_store(
        std::function<void(basic_store_message_variant<PacketIdBytes>)> const&
            f) {
        MQTT_LOG("mqtt_api", info) << MQTT_ADD_VALUE(address, this)
     ...
     */
    void for_each_store_with_life_keeper(std::function<void(
            basic_store_message_variant<PacketIdBytes>, any)> const& f) {
        MQTT_LOG("mqtt_api", info)
     ...
     *        If all packet ids are already in use, then throw
     * packet_id_exhausted_error exception. After acquiring the packet id, you
     * can call acquired_* functions. The ownership of packet id is moved to the
     * library. Or you can call release_packet_id to release it.
     * @return packet id
     */
     ...
     *        After registering the packet_id, you can call acquired_*
     * functions. The ownership of packet id is moved to the library. Or you can
     * call release_packet_id to release it.
     * @return If packet_id is successfully registerd then return true,
     ...
     *                   only the packet_id gotten by acquire_unique_packet_id,
     * or register_packet_id is permitted.
     */
    ...
    std::enable_if_t<
        std::is_convertible<typename Iterator::value_type, char>::value>
    restore_serialized_message(Iterator b, Iterator e) {
        ...
        static_assert(
            ...
            "Iterators provided to restore_serialized_message() must be random "
            "access iterators.");
        ...
        if (!cpt_opt) {
            MQTT_LOG("mqtt_api", error)
                << MQTT_ADD_VALUE(address, this)
                << "invalid fixed_header ignored. " << std::hex
                << static_cast<int>(fixed_header);
        ...
        switch (cpt_opt.value()) {
            case control_packet_type::publish: {
                ...
                restore_serialized_message(
                    basic_publish_message<PacketIdBytes>(buf), buf);
            } break;
            case control_packet_type::pubrel: {
                restore_serialized_message(basic_pubrel_message<PacketIdBytes>(
                    // basic_pubrel_message have no member variable that type is
                    // buffer. When creating basic_pubrel_message, the
                    // constructor just read buffer and convert to some values.
                    // So the argument buffer(...) doesn't need to hold the
                    // lifetime.
                    buffer(string_view(
                        &*b, static_cast<std::size_t>(std::distance(b, e))))));
            } break;
            default:
                MQTT_LOG("mqtt_api", error)
                    << MQTT_ADD_VALUE(address, this)
                    << "invalid control packet type. " << std::hex
                    << static_cast<int>(fixed_header);
     ...
     * @param msg         publish message.
     * @param life_keeper
     *        An object that stays alive (but is moved with force_move()) until
     * the stored message is sent.
     */
    void restore_serialized_message(
        basic_publish_message<PacketIdBytes> msg, any life_keeper = {}) {
        BOOST_ASSERT(version_ == protocol_version::v3_1_1);
        auto packet_id = msg.packet_id();
        qos qos_value = msg.get_qos();
        LockGuard<Mutex> lck(store_mtx_);
        if (pid_man_.register_id(packet_id)) {
            auto ret = store_.emplace(packet_id,
                ((qos_value == qos::at_least_once)
                        ? control_packet_type::puback
                        : control_packet_type::pubrec),
                force_move(msg), force_move(life_keeper));
            ...
            if (!ret.second) {
                store_.modify(ret.first, [&](auto& e) {
                    e = store(packet_id,
                        ((qos_value == qos::at_least_once)
                                ? control_packet_type::puback
                                : control_packet_type::pubrec),
     ...
     */
    void restore_serialized_message(
        basic_pubrel_message<PacketIdBytes> msg, any life_keeper = {}) {
        BOOST_ASSERT(version_ == protocol_version::v3_1_1);
        auto packet_id = msg.packet_id();
        LockGuard<Mutex> lck(store_mtx_);
        if (pid_man_.register_id(packet_id)) {
            auto ret = store_.emplace(packet_id, control_packet_type::pubcomp,
                force_move(msg), force_move(life_keeper));
            // When client want to restore serialized messages,
            // endpoint might keep the message that has the same packet_id.
            // In this case, overwrite store_.
            if (!ret.second) {
                store_.modify(ret.first, [&](auto& e) {
                    e = store(packet_id, control_packet_type::pubcomp,
                        force_move(msg), force_move(life_keeper));
                });
            }
        }
        ...
        switch (cpt_opt.value()) {
            case control_packet_type::publish: {
                ...
                restore_v5_serialized_message(
                    v5::basic_publish_message<PacketIdBytes>(buf), buf);
            } break;
            case control_packet_type::pubrel: {
                auto buf = allocate_buffer(b, e);
                restore_v5_serialized_message(
                    v5::basic_pubrel_message<PacketIdBytes>(buf), buf);
            } break;
            default:
                MQTT_LOG("mqtt_api", error)
                    << MQTT_ADD_VALUE(address, this)
     ...
     * the stored message is sent.
     */
    void restore_v5_serialized_message(
        v5::basic_publish_message<PacketIdBytes> msg, any life_keeper = {}) {
        BOOST_ASSERT(version_ == protocol_version::v5);
     ...
     */
    void restore_v5_serialized_message(
        v5::basic_pubrel_message<PacketIdBytes> msg, any life_keeper = {}) {
        BOOST_ASSERT(version_ == protocol_version::v5);
        auto packet_id = msg.packet_id();
        LockGuard<Mutex> lck(store_mtx_);
        if (pid_man_.register_id(packet_id)) {
   ...
   private:
    struct restore_basic_message_variant_visitor {
        restore_basic_message_variant_visitor(this_type& ep, any life_keeper)
            : ep_(ep), life_keeper_(force_move(life_keeper)) {}

        void operator()(basic_publish_message<PacketIdBytes>&& msg) {
            ep_.restore_serialized_message(
                force_move(msg), force_move(life_keeper_));
        }
        ...
        void operator()(v5::basic_publish_message<PacketIdBytes>&& msg) {
            ep_.restore_v5_serialized_message(
                force_move(msg), force_move(life_keeper_));
        }
   ...
   public:
    void restore_serialized_message(
        basic_store_message_variant<PacketIdBytes> msg, any life_keeper = {}) {
        MQTT_NS::visit(restore_basic_message_variant_visitor(
                           *this, force_move(life_keeper)),
            force_move(msg));
    }

    void send_store_message(
        basic_store_message_variant<PacketIdBytes> msg, any life_keeper) {
        auto publish_proc = [&](auto msg, auto&& serialize,
                                auto&& receive_maximum_proc) {
            auto msg_lk = apply_topic_alias(msg, life_keeper);
            if (maximum_packet_size_send_ <
                size<PacketIdBytes>(std::get<0>(msg_lk))) {
                throw packet_size_error();
            }
            if (preprocess_publish_message(msg, life_keeper,
                    std::forward<decltype(serialize)>(serialize),
                    std::forward<decltype(receive_maximum_proc)>(
        ...
        auto pubrel_proc = [&](auto msg, auto const& serialize) {
        ...
        MQTT_NS::visit(
            make_lambda_visitor(
                ...
                [this, &publish_proc](
                    ...
                    publish_proc(force_move(m),
                        ...
                        [this](v5::basic_publish_message<PacketIdBytes>&& msg) {
                            if (publish_send_count_.load() ==
                                publish_send_max_) {
                                LockGuard<Mutex> lck(publish_send_queue_mtx_);
                                publish_send_queue_.emplace_back(
                                    force_move(msg), false);
                                return false;
                            } else {
                                MQTT_LOG("mqtt_impl", trace)
                                    << MQTT_ADD_VALUE(address, this)
                                    << "increment publish_send_count_:"
                                    << publish_send_count_.load();
                                ++publish_send_count_;
                            }
                            return true;
                        });
                ...
                [this, &pubrel_proc](
                    ...
                    pubrel_proc(force_move(m),
                        &endpoint::on_serialize_v5_pubrel_message);
                }),
            msg);
    }

    void async_send_store_message(
        basic_store_message_variant<PacketIdBytes> msg, any life_keeper,
        async_handler_t func) {
        auto publish_proc = [&](auto msg, auto&& serialize,
                                auto&& receive_maximum_proc) {
            ...
            if (preprocess_publish_message(msg, life_keeper,
                ...
                do_async_write(force_move(std::get<0>(msg_lk)),
                    [func = force_move(func),
                        life_keeper = force_move(std::get<1>(msg_lk))](
                        error_code ec) {
                        if (func) func(ec);
                    });
            }
        ...
        MQTT_NS::visit(
            make_lambda_visitor(
                ...
                [this, &publish_proc, &func](
                    ...
                    publish_proc(force_move(m),
                        &endpoint::on_serialize_v5_publish_message,
                        [this, func](v5::basic_publish_message<PacketIdBytes>&&
                                msg) mutable {
                            if (publish_send_count_.load() ==
                                publish_send_max_) {
                                {
                                    LockGuard<Mutex> lck(
                                        publish_send_queue_mtx_);
                                    publish_send_queue_.emplace_back(
                                        force_move(msg), true);
                                }
                                socket_->post([func = force_move(func)] {
                                    // message has already been stored so func
                                    // should be called with success here
                                    if (func)
                                        func(boost::system::errc::
                                                make_error_code(boost::system::
                                                        errc::success));
                                });
                                return false;
                            } else {
     ...
     */
    bool connected() const { return connected_ && mqtt_connected_; }

    /**
     * @brief Check underlying layer connection status
     * @return current connection status
     */
    bool underlying_connected() const { return connected_; }
    ...
    protocol_version get_protocol_version() const { return version_; }

    MQTT_NS::socket const& socket() const { return *socket_; }

    MQTT_NS::socket& socket() { return *socket_; }
     ...
     * The timer cancels when PINGRESP is received. If the timer is fired, then
     * force_disconnect from the client side.<BR>
     * https://docs.oasis-open.org/mqtt/mqtt/v5.0/os/mqtt-v5.0-os.html#_Toc3901045<BR>
     * 3.1.2.10 Keep Alive
     */
     ...
     * If property is manually set, then maximum_packet_size_recv_ is
     * overwritten by the property.
     *
     ...
     * If the endpoint is client, then it sends as CONNECT packet property.
     * If the endpoint is server, then it sends as CONNACK packet property.
     * If property is manually set, then publish_recv_max_ is overwritten by the
     * property.
     *
     * @param size maximum packet size
     */
   ...
   protected:
     ...
     */
    std::shared_ptr<MQTT_NS::socket>& socket_sp_ref() { return socket_; }

    void async_read_control_packet_type(any session_life_keeper) {
        socket_->async_read(as::buffer(buf_.data(), 1),
            [this, self = this->shared_from_this(),
                ...
                if (!check_error_and_transferred_length(
                        ec, bytes_transferred, 1))
                    return;
                handle_control_packet_type(
                    force_move(session_life_keeper), force_move(self));
            });
    }

    bool handle_close_or_error(error_code ec) {
        auto call_handler = [this, ec]() mutable {
        ...
        if (connected_) {
            ...
            if (async_operation_) {
                async_shutdown(socket(),
                    [call_handler](error_code) mutable { call_handler(); });
            } else {
                sync_shutdown(socket());
                call_handler();
            }
    ...
    void set_connect() { connected_ = true; }

    void set_protocol_version(protocol_version version) { version_ = version; }
    ...
    void set_client_id(protocol_version version) { version_ = version; }
    ...
    void update_values_and_props_on_start_connection(v5::properties& props) {
        ...
        if (topic_alias_maximum_count == 0) {
            ...
            if (topic_alias_recv_ && topic_alias_recv_.value().max() != 0) {
                props.emplace_back(MQTT_NS::v5::property::topic_alias_maximum(
                    topic_alias_recv_.value().max()));
            }
        }
        if (maximum_packet_size_count == 0) {
            if (maximum_packet_size_recv_ != packet_size_no_limit) {
                props.emplace_back(MQTT_NS::v5::property::maximum_packet_size(
                    static_cast<std::uint32_t>(maximum_packet_size_recv_)));
            }
        }
        if (receive_maximum_count == 0) {
            if (publish_recv_max_ != receive_maximum_max) {
                props.emplace_back(MQTT_NS::v5::property::receive_maximum(
                    static_cast<receive_maximum_t>(publish_recv_max_)));
            }
        }
    }

    bool set_values_from_props_on_connection(
        connection_type type, v5::properties const& props) {
...
#undef MQTT_SEND_ERROR
    ...
    bool check_transferred_length(
        std::size_t bytes_transferred, std::size_t bytes_expected) {
    ...
    bool check_error_and_transferred_length(error_code ec,
        std::size_t bytes_transferred, std::size_t bytes_expected) {
        if (handle_close_or_error(ec)) return false;
        if (!check_transferred_length(bytes_transferred, bytes_expected))
            return false;
        return true;
    }

    void call_bad_message_error_handlers() {
        clean_sub_unsub_inflight_on_error(boost::system::errc::make_error_code(
    ...
    void sync_shutdown(MQTT_NS::socket& s) {
        MQTT_LOG("mqtt_impl", trace)
            << MQTT_ADD_VALUE(address, this) << "sync_shutdown";
        if (shutdown_requested_) {
            MQTT_LOG("mqtt_impl", trace)
                << MQTT_ADD_VALUE(address, this) << "already shutdowned";
            return;
        }
        ...
        error_code ec;
        MQTT_LOG("mqtt_impl", trace)
            << MQTT_ADD_VALUE(address, this) << "clean_shutdown_and_close";
        s.clean_shutdown_and_close(ec);
        MQTT_LOG("mqtt_impl", trace)
            << MQTT_ADD_VALUE(address, this)
            << "clean_shutdown_and_close ec:" << ec.message();
        connected_ = false;
    }

    void async_shutdown(MQTT_NS::socket& s, async_handler_t func) {
        MQTT_LOG("mqtt_impl", trace)
            << MQTT_ADD_VALUE(address, this) << "shutdown";
        if (shutdown_requested_) {
            MQTT_LOG("mqtt_impl", trace)
                << MQTT_ADD_VALUE(address, this) << "already shutdowned";
            if (func)
        ...
        MQTT_LOG("mqtt_impl", trace) << MQTT_ADD_VALUE(address, this)
                                     << "async_clean_shutdown_and_close";
        s.async_clean_shutdown_and_close(
            [this, func, sp = this->shared_from_this(), ssp = socket_sp_ref()](
                error_code ec) {  // *1
                MQTT_LOG("mqtt_impl", trace)
                    << MQTT_ADD_VALUE(address, this)
                    << "async_clean_shutdown_and_close ec:" << ec.message();
        ...
        std::weak_ptr<this_type> wp(
            std::static_pointer_cast<this_type>(this->shared_from_this()));
        tim_shutdown_.async_wait(
            [this, func, wp = force_move(wp), ssp = socket_sp_ref()](
                error_code ec) mutable {
                if (auto sp = wp.lock()) {
                    MQTT_LOG("mqtt_impl", trace)
                        << MQTT_ADD_VALUE(address, this)
                        << "async_shutdown timer ec:" << ec.message();
                    if (!ec) {
                        // timeout
                        // tcp_shutdown indirectly cancel
                        // stream.async_shutdown() and handler is called with
                        // error. So captured sp at *1 is released.

                        // post is for applying strand
                        MQTT_LOG("mqtt_impl", trace)
                            << MQTT_ADD_VALUE(address, this)
                            << "post force_shutdown_and_close";
                        sp->socket().post([this, func = force_move(func), sp] {
                            if (connected_) {
                                error_code ec;
    ...
    void send_error_disconnect(v5::disconnect_reason_code rc) {
        if (async_operation_) {
            ...
            async_disconnect(rc, v5::properties{},
                [sp = force_move(sp)](error_code) mutable {
                    sp->socket().post(
                        [sp = force_move(sp)] { sp->force_disconnect(); });
                });
        } else {
            disconnect(rc);
            force_disconnect();
    ...
    void send_error_connack(v5::connect_reason_code rc) {
        if (async_operation_) {
            auto sp = this->shared_from_this();
            async_connack(false, rc, [sp = force_move(sp)](error_code) mutable {
                sp->socket().post(
                    [sp = force_move(sp)] { sp->force_disconnect(); });
            });
        } else {
       ...
       public:
        send_buffer()
            : buf_(std::make_shared<std::string>(
                  static_cast<int>(payload_position_), 0)) {}

        std::shared_ptr<std::string> const& buf() const { return buf_; }

        std::shared_ptr<std::string>& buf() { return buf_; }
        ...
        std::pair<char*, std::size_t> finalize(std::uint8_t fixed_header) {
            ...
            return std::make_pair(
                &(*buf_)[start_position], buf_->size() - start_position);
        }
    ...
    struct store {
        store(packet_id_t id, control_packet_type type,
        ...
        control_packet_type expected_control_packet_type() const {
            return expected_control_packet_type_;
        }
        basic_store_message_variant<PacketIdBytes> const& message() const {
            return smv_;
        }
        basic_store_message_variant<PacketIdBytes>& message() { return smv_; }
        any const& life_keeper() const { return life_keeper_; }
    ...
    struct tag_seq {};
    using mi_store = mi::multi_index_container<store,
        mi::indexed_by<
            mi::ordered_unique<mi::tag<tag_packet_id_type>,
                mi::composite_key<store,
                    mi::const_mem_fun<store, packet_id_t, &store::packet_id>,
                    mi::const_mem_fun<store, control_packet_type,
                        &store::expected_control_packet_type>>>,
            mi::ordered_non_unique<mi::tag<tag_packet_id>,
                mi::const_mem_fun<store, packet_id_t, &store::packet_id>>,
            mi::sequenced<mi::tag<tag_seq>>>>;

    void handle_control_packet_type(
        any session_life_keeper, this_type_sp self) {
    ...
    bool calc_variable_length(
        std::size_t& v, std::size_t& multiplier, char buf) {
    ...
    void handle_remaining_length(any session_life_keeper, this_type_sp self) {
        if (!calc_variable_length(remaining_length_,
                remaining_length_multiplier_, buf_.front())) {
            call_bad_message_error_handlers();
            return;
        }
                    ...
                    case protocol_version::v3_1_1:
                        switch (cpt) {
                            ...
                            case control_packet_type::unsubscribe:
                                return check_is_valid_length(
                                    cpt, remaining_length_);
                            ...
                            // Even though there is no auth packet type in
                            // v3.1.1 it's included in the switch case to
                            // provide a warning about missing enum values if
                            // any are missing.
    ...
    void process_payload(any session_life_keeper, this_type_sp self) {
        ...
        switch (control_packet_type) {
            case control_packet_type::connect:
                (*std::make_shared<process_connect>(
                    *this, remaining_length_ < packet_bulk_read_limit_))(
                    force_move(self), force_move(session_life_keeper));
                break;
            case control_packet_type::connack:
                (*std::make_shared<process_connack>(
                    *this, remaining_length_ < packet_bulk_read_limit_))(
                    force_move(self), force_move(session_life_keeper));
                break;
            case control_packet_type::publish:
                if (mqtt_connected_) {
                    (*std::make_shared<process_publish>(
                        *this, remaining_length_ < packet_bulk_read_limit_))(
                        force_move(self), force_move(session_life_keeper));
                } else {
                    call_protocol_error_handlers();
            ...
            case control_packet_type::puback:
                if (mqtt_connected_) {
                    (*std::make_shared<process_puback>(
                        *this, remaining_length_ < packet_bulk_read_limit_))(
                        force_move(self), force_move(session_life_keeper));
                } else {
                    call_protocol_error_handlers();
            ...
            case control_packet_type::pubrec:
                if (mqtt_connected_) {
                    (*std::make_shared<process_pubrec>(
                        *this, remaining_length_ < packet_bulk_read_limit_))(
                        force_move(self), force_move(session_life_keeper));
                } else {
                    call_protocol_error_handlers();
            ...
            case control_packet_type::pubrel:
                if (mqtt_connected_) {
                    (*std::make_shared<process_pubrel>(
                        *this, remaining_length_ < packet_bulk_read_limit_))(
                        force_move(self), force_move(session_life_keeper));
                } else {
                    call_protocol_error_handlers();
            ...
            case control_packet_type::pubcomp:
                if (mqtt_connected_) {
                    (*std::make_shared<process_pubcomp>(
                        *this, remaining_length_ < packet_bulk_read_limit_))(
                        force_move(self), force_move(session_life_keeper));
                } else {
                    call_protocol_error_handlers();
            ...
            case control_packet_type::subscribe:
                if (mqtt_connected_) {
                    (*std::make_shared<process_subscribe>(
                        *this, remaining_length_ < packet_bulk_read_limit_))(
                        force_move(self), force_move(session_life_keeper));
                } else {
                    call_protocol_error_handlers();
            ...
            case control_packet_type::suback:
                if (mqtt_connected_) {
                    (*std::make_shared<process_suback>(
                        *this, remaining_length_ < packet_bulk_read_limit_))(
                        force_move(self), force_move(session_life_keeper));
                } else {
                    call_protocol_error_handlers();
            ...
            case control_packet_type::unsubscribe:
                if (mqtt_connected_) {
                    (*std::make_shared<process_unsubscribe>(
                        *this, remaining_length_ < packet_bulk_read_limit_))(
                        force_move(self), force_move(session_life_keeper));
                } else {
                    call_protocol_error_handlers();
            ...
            case control_packet_type::unsuback:
                if (mqtt_connected_) {
                    (*std::make_shared<process_unsuback>(
                        *this, remaining_length_ < packet_bulk_read_limit_))(
                        force_move(self), force_move(session_life_keeper));
                } else {
                    call_protocol_error_handlers();
            ...
            case control_packet_type::pingresp:
                ...
                break;
            case control_packet_type::disconnect:
                (*std::make_shared<process_disconnect>(
                    *this, remaining_length_ < packet_bulk_read_limit_))(
                    force_move(self), force_move(session_life_keeper));
                break;
            case control_packet_type::auth:
                (*std::make_shared<process_auth>(
                    *this, remaining_length_ < packet_bulk_read_limit_))(
                    force_move(self), force_move(session_life_keeper));
                break;
            default:
    ...
    using parse_handler_variant =
        variant<std::conditional_t<sizeof(std::size_t) == 4, std::nullptr_t,
                    std::size_t>,
            std::uint32_t, std::uint16_t, buffer, v5::properties>;
    using parse_handler = std::function<void(this_type_sp&& spep,
        any&& session_life_keeper, parse_handler_variant, buffer)>;

    // primitive read functions
    void process_nbytes(this_type_sp&& self, any&& session_life_keeper,
        buffer buf, std::size_t size, parse_handler&& handler) {
        ...
        if (buf.empty()) {
            ...
            auto ptr = spa.get();
            socket_->async_read(as::buffer(ptr, size),
                [this, self = force_move(self),
                    session_life_keeper = force_move(session_life_keeper),
                    ...
                    this->total_bytes_received_ += bytes_transferred;
                    if (!check_error_and_transferred_length(
                            ec, bytes_transferred, buf.size()))
                        return;
                    handler(force_move(self), force_move(session_life_keeper),
                        force_move(buf), buffer());
                });
        } else {
            if (buf.size() < size) {
                call_protocol_error_handlers();
            ...
            handler(force_move(self), force_move(session_life_keeper),
                buf.substr(0, size), buf.substr(size));
        }
    }

    template <std::size_t Bytes>
    void process_fixed_length(this_type_sp&& self, any&& session_life_keeper,
        buffer buf, parse_handler&& handler) {
        ...
        if (buf.empty()) {
            socket_->async_read(as::buffer(buf_.data(), Bytes),
                [this, self = force_move(self),
                    session_life_keeper = force_move(session_life_keeper),
                    handler = force_move(handler)](
                    error_code ec, std::size_t bytes_transferred) mutable {
                    this->total_bytes_received_ += bytes_transferred;
                    if (!check_error_and_transferred_length(
                            ec, bytes_transferred, Bytes))
                        return;
                    handler(force_move(self), force_move(session_life_keeper),
                        make_two_or_four_byte<Bytes>::apply(buf_.data(),
                            std::next(buf_.data(),
                                boost::numeric_cast<buffer::difference_type>(
                                    Bytes))),
                        buffer());
                });
        } else {
            auto val = make_two_or_four_byte<Bytes>::apply(buf.data(),
                std::next(buf.data(),
                    boost::numeric_cast<buffer::difference_type>(Bytes)));
            buf.remove_prefix(Bytes);
            handler(force_move(self), force_move(session_life_keeper), val,
                force_move(buf)
    ...
    void process_variable_length(this_type_sp&& self, any&& session_life_keeper,
        buffer buf, parse_handler&& handler) {
        process_variable_length_impl(force_move(self),
            force_move(session_life_keeper), force_move(buf),
            force_move(handler), 0, 1);
    }

    void process_variable_length_impl(this_type_sp&& self,
        any&& session_life_keeper, buffer buf, parse_handler&& handler,
        std::size_t size, std::size_t multiplier) {
        ...
        auto proc = [this](this_type_sp&& self, any&& session_life_keeper,
                        buffer&& buf, auto&& handler, std::size_t size,
                        std::size_t multiplier) mutable {
            ...
            if (buf.front() & variable_length_continue_flag) {
                ...
                process_variable_length_impl(force_move(self),
                    force_move(session_life_keeper), force_move(buf),
                    std::forward<decltype(handler)>(handler), size, multiplier);
            } else {
                buf.remove_prefix(1);
                handler(force_move(self), force_move(session_life_keeper), size,
                    force_move(buf));
            }
        };
        ...
        if (buf.empty()) {
            socket_->async_read(as::buffer(buf_.data(), 1),
                [this, self = force_move(self),
                    session_life_keeper = force_move(session_life_keeper),
                    handler = force_move(handler), size, multiplier,
                    ...
                    if (!check_error_and_transferred_length(
                            ec, bytes_transferred, 1))
                        return;
                    proc(force_move(self), force_move(session_life_keeper),
                        buffer(string_view(buf_.data(),
                            1)),  // buf_'s lifetime is handled by `self`
                        force_move(handler), size, multiplier);
                });
        } else {
            proc(force_move(self), force_move(session_life_keeper),
                force_move(buf), force_move(handler), size, multiplier);
        }
    }

    void process_packet_id(this_type_sp&& self, any&& session_life_keeper,
        buffer buf, parse_handler&& handler) {
        process_fixed_length<sizeof(packet_id_t)>(force_move(self),
            force_move(session_life_keeper), force_move(buf),
            [this, handler = force_move(handler)](this_type_sp&& self,
                any&& session_life_keeper, parse_handler_variant var,
                buffer buf) mutable {
                ...
                handler(force_move(self), force_move(session_life_keeper),
                    static_cast<packet_id_t>(packet_id), force_move(buf));
            });
    }

    void process_binary(this_type_sp&& self, any&& session_life_keeper,
        buffer buf, parse_handler&& handler) {
        if (remaining_length_ < 2) {
            call_protocol_error_handlers();
            return;
        }
        process_fixed_length<2>(force_move(self),
            force_move(session_life_keeper), force_move(buf),
            [this, handler = force_move(handler)](this_type_sp&& self,
                any&& session_life_keeper, parse_handler_variant var,
                buffer buf) mutable {
                auto size =
                    variant_get<typename two_or_four_byte_type<2>::type>(var);
                if (remaining_length_ < size) {
                    call_protocol_error_handlers();
                    return;
                }
                process_nbytes(force_move(self),
                    force_move(session_life_keeper), force_move(buf), size,
                    force_move(handler));
            });
    }

    void process_string(this_type_sp&& self, any&& session_life_keeper,
        buffer buf, parse_handler&& handler) {
        process_binary(force_move(self), force_move(session_life_keeper),
            force_move(buf),
            [this, handler = force_move(handler)](this_type_sp&& self,
                any&& session_life_keeper, parse_handler_variant var,
                buffer buf) mutable {
                ...
                handler(force_move(self), force_move(session_life_keeper),
                    force_move(str), force_move(buf));
            });
    }

    void process_properties(this_type_sp&& self, any&& session_life_keeper,
        buffer buf, parse_handler&& handler) {
        process_variable_length(force_move(self),
            force_move(session_life_keeper), force_move(buf),
            [this, handler = force_move(handler)](this_type_sp&& self,
                any&& session_life_keeper, parse_handler_variant var,
                buffer buf) mutable {
                ...
                if (buf.empty()) {
                    ...
                    auto result = [&]() -> spa_address_len {
                        if (property_length < props_bulk_read_limit_) {
                            auto spa = make_shared_ptr_array(property_length);
                            auto ptr = spa.get();
                            return {force_move(spa), ptr, property_length};
                        }
                        return {nullptr, buf_.data(), 1};
                    }();
                    socket_->async_read(as::buffer(result.address, result.len),
                        [this, handler = force_move(handler),
                            self = force_move(self),
                            session_life_keeper =
                                force_move(session_life_keeper),
                            property_length, result](error_code ec,
                            std::size_t bytes_transferred) mutable {
                            this->total_bytes_received_ += bytes_transferred;
                            if (!check_error_and_transferred_length(
                                    ec, bytes_transferred, result.len))
                                return;
                            process_property_id(force_move(self),
                                force_move(session_life_keeper),
                                buffer(string_view(result.address, result.len),
                                    result.spa),
                                property_length, v5::properties(),
                                force_move(handler));
                        });
                } else {
                    process_property_id(force_move(self),
                        force_move(session_life_keeper), force_move(buf),
                        property_length, v5::properties(), force_move(handler));
                }
            });
    }

    void process_property_id(this_type_sp&& self, any&& session_life_keeper,
        buffer buf, std::size_t property_length_rest, v5::properties props,
        parse_handler&& handler) {
        if (property_length_rest == 0) {
            handler(force_move(self), force_move(session_life_keeper),
                force_move(props), force_move(buf));
            return;
        }
        ...
        if (buf.empty()) {
            socket_->async_read(as::buffer(buf_.data(), 1),
                [this, self = force_move(self),
                    session_life_keeper = force_move(session_life_keeper),
                    props = force_move(props), handler = force_move(handler),
                    ...
                    if (!check_error_and_transferred_length(
                            ec, bytes_transferred, 1))
                        return;
                    process_property_body(force_move(self),
                        force_move(session_life_keeper), buffer(),
                        static_cast<v5::property::id>(buf_.front()),
                        property_length_rest - 1, force_move(props),
                        force_move(handler));
                });
        } else {
            ...
            buf.remove_prefix(1);
            process_property_body(force_move(self),
                force_move(session_life_keeper), force_move(buf), id,
                property_length_rest - 1, force_move(props),
                force_move(handler));
        }
    }

    void process_property_body(this_type_sp&& self, any&& session_life_keeper,
        buffer buf, v5::property::id id, std::size_t property_length_rest,
        v5::properties props, parse_handler&& handler) {
        ...
        switch (id) {
            case v5::property::id::payload_format_indicator: {
                ...
                process_nbytes(force_move(self),
                    force_move(session_life_keeper), force_move(buf), len,
                    [this, props = force_move(props),
                        handler = force_move(handler),
                        rest = property_length_rest - len](this_type_sp&& self,
                        any&& session_life_keeper, parse_handler_variant var,
                        buffer buf) mutable {
                        ...
                        process_property_id(force_move(self),
                            force_move(session_life_keeper), force_move(buf),
                            rest, force_move(props), force_move(handler));
                    });
            } break;
            ...
            case v5::property::id::content_type: {
                process_string(force_move(self),
                    force_move(session_life_keeper), force_move(buf),
                    [this, props = force_move(props),
                        handler = force_move(handler), property_length_rest](
                        this_type_sp&& self, any&& session_life_keeper,
                        parse_handler_variant var, buffer buf) mutable {
                        auto& body = variant_get<buffer>(var);
                        auto rest =
                            property_length_rest - length_bytes - body.size();
                        props.emplace_back(
            ...
            case v5::property::id::response_topic: {
                process_string(force_move(self),
                    ...
                    [this, props = force_move(props),
                        ...
                        auto rest =
                            property_length_rest - length_bytes - body.size();
                        props.emplace_back(v5::property::response_topic(
                            force_move(body), true));
                        process_property_id(force_move(self),
                            force_move(session_life_keeper), force_move(buf),
                            rest, force_move(props), force_move(handler));
                    });
            ...
            case v5::property::id::correlation_data: {
                process_binary(force_move(self),
                    force_move(session_life_keeper), force_move(buf),
                    [this, props = force_move(props),
                        handler = force_move(handler), property_length_rest](
                        this_type_sp&& self, any&& session_life_keeper,
            ...
            case v5::property::id::session_expiry_interval: {
                ...
                process_nbytes(force_move(self),
                    ...
                    [this, props = force_move(props),
                        ...
                        auto& body = variant_get<buffer>(var);
                        props.emplace_back(
                            v5::property::session_expiry_interval(
                                body.begin(), body.end()));
                        process_property_id(force_move(self),
                            force_move(session_life_keeper), force_move(buf),
                            rest, force_move(props), force_move(handler));
                ...
                process_string(force_move(self),
                    ...
                    [this, props = force_move(props),
                        ...
                        auto rest =
                            property_length_rest - length_bytes - body.size();
                        props.emplace_back(
                            v5::property::assigned_client_identifier(
                                force_move(body), true));
                        process_property_id(force_move(self),
                            force_move(session_life_keeper), force_move(buf),
                            rest, force_move(props), force_move(handler));
            ...
            case v5::property::id::server_keep_alive: {
                ...
                process_nbytes(force_move(self),
                    ...
                    [this, props = force_move(props),
                        ...
                        buffer buf) mutable {
                        auto& body = variant_get<buffer>(var);
                        props.emplace_back(v5::property::server_keep_alive(
                            body.begin(), body.end()));
                        process_property_id(force_move(self),
                            force_move(session_life_keeper), force_move(buf),
                            rest, force_move(props), force_move(handler));
            ...
            case v5::property::id::authentication_method: {
                process_string(force_move(self),
                    ...
                    [this, props = force_move(props),
                        ...
                        auto rest =
                            property_length_rest - length_bytes - body.size();
                        props.emplace_back(v5::property::authentication_method(
                            force_move(body), true));
                        process_property_id(force_move(self),
                            force_move(session_life_keeper), force_move(buf),
                            rest, force_move(props), force_move(handler));
            ...
            case v5::property::id::authentication_data: {
                process_binary(force_move(self),
                    ...
                    [this, props = force_move(props),
                        ...
                        auto rest =
                            property_length_rest - length_bytes - body.size();
                        props.emplace_back(v5::property::authentication_data(
                            force_move(body)));
                        process_property_id(force_move(self),
                            force_move(session_life_keeper), force_move(buf),
                            rest, force_move(props), force_move(handler));
            ...
            case v5::property::id::request_problem_information: {
                ...
                process_nbytes(force_move(self),
                    ...
                    [this, props = force_move(props),
                        ...
                        auto& body = variant_get<buffer>(var);
                        props.emplace_back(
                            v5::property::request_problem_information(
                                body.begin(), body.end()));
                        process_property_id(force_move(self),
                            force_move(session_life_keeper), force_move(buf),
                            rest, force_move(props), force_move(handler));
            ...
            case v5::property::id::will_delay_interval: {
                ...
                process_nbytes(force_move(self),
                    ...
                    [this, props = force_move(props),
                        ...
                        buffer buf) mutable {
                        auto& body = variant_get<buffer>(var);
                        props.emplace_back(v5::property::will_delay_interval(
                            body.begin(), body.end()));
                        process_property_id(force_move(self),
                            force_move(session_life_keeper), force_move(buf),
                            rest, force_move(props), force_move(handler));
            ...
            case v5::property::id::request_response_information: {
                ...
                process_nbytes(force_move(self),
                    ...
                    [this, props = force_move(props),
                        ...
                        auto& body = variant_get<buffer>(var);
                        props.emplace_back(
                            v5::property::request_response_information(
                                body.begin(), body.end()));
                        process_property_id(force_move(self),
                            force_move(session_life_keeper), force_move(buf),
                            rest, force_move(props), force_move(handler));
            ...
            case v5::property::id::server_reference: {
                process_string(force_move(self),
                    ...
                    [this, props = force_move(props),
                        ...
                        auto rest =
                            property_length_rest - length_bytes - body.size();
                        props.emplace_back(v5::property::server_reference(
                            force_move(body), true));
                        process_property_id(force_move(self),
                            force_move(session_life_keeper), force_move(buf),
                            rest, force_move(props), force_move(handler));
            ...
            case v5::property::id::reason_string: {
                process_string(force_move(self),
                    ...
                    [this, props = force_move(props),
                        ...
                        auto rest =
                            property_length_rest - length_bytes - body.size();
                        props.emplace_back(v5::property::reason_string(
                            force_move(body), true));
                        process_property_id(force_move(self),
                            force_move(session_life_keeper), force_move(buf),
                            rest, force_move(props), force_move(handler));
            ...
            case v5::property::id::receive_maximum: {
                ...
                process_nbytes(force_move(self),
                    ...
                    [this, props = force_move(props),
                        ...
                        buffer buf) mutable {
                        auto& body = variant_get<buffer>(var);
                        props.emplace_back(v5::property::receive_maximum(
                            body.begin(), body.end()));
                        process_property_id(force_move(self),
                            force_move(session_life_keeper), force_move(buf),
                            rest, force_move(props), force_move(handler));
            ...
            case v5::property::id::topic_alias_maximum: {
                ...
                process_nbytes(force_move(self),
                    ...
                    [this, props = force_move(props),
                        ...
                        buffer buf) mutable {
                        auto& body = variant_get<buffer>(var);
                        props.emplace_back(v5::property::topic_alias_maximum(
                            body.begin(), body.end()));
                        process_property_id(force_move(self),
                            force_move(session_life_keeper), force_move(buf),
                            rest, force_move(props), force_move(handler));
            ...
            case v5::property::id::topic_alias: {
                ...
                process_nbytes(force_move(self),
                    ...
                    [this, props = force_move(props),
                        ...
                        buffer buf) mutable {
                        auto& body = variant_get<buffer>(var);
                        props.emplace_back(v5::property::topic_alias(
                            body.begin(), body.end()));
                        process_property_id(force_move(self),
                            force_move(session_life_keeper), force_move(buf),
                            rest, force_move(props), force_move(handler));
            ...
            case v5::property::id::maximum_qos: {
                ...
                process_nbytes(force_move(self),
                    ...
                    [this, props = force_move(props),
                        ...
                        buffer buf) mutable {
                        auto& body = variant_get<buffer>(var);
                        props.emplace_back(v5::property::maximum_qos(
                            body.begin(), body.end()));
                        process_property_id(force_move(self),
                            force_move(session_life_keeper), force_move(buf),
                            rest, force_move(props), force_move(handler));
            ...
            case v5::property::id::retain_available: {
                ...
                process_nbytes(force_move(self),
                    ...
                    [this, props = force_move(props),
                        ...
                        buffer buf) mutable {
                        auto& body = variant_get<buffer>(var);
                        props.emplace_back(v5::property::retain_available(
                            body.begin(), body.end()));
                        process_property_id(force_move(self),
                            force_move(session_life_keeper), force_move(buf),
                            rest, force_move(props), force_move(handler));
            ...
            case v5::property::id::user_property: {
                process_string(force_move(self),
                    ...
                    [this, props = force_move(props),
                        ...
                        auto rest =
                            property_length_rest - length_bytes - key.size();
                        process_string(force_move(self),
                            force_move(session_life_keeper), force_move(buf),
                            [this, props = force_move(props),
                                handler = force_move(handler),
                                ...
                                auto rest = property_length_rest -
                                            length_bytes - val.size();
                                props.emplace_back(
                                    v5::property::user_property(force_move(key),
                                        force_move(val), true, true));
                                process_property_id(force_move(self),
                                    force_move(session_life_keeper),
                                    force_move(buf), rest, force_move(props),
                                    force_move(handler));
                            });
                    });
            } break;
            case v5::property::id::maximum_packet_size: {
                ...
                process_nbytes(force_move(self),
                    ...
                    [this, props = force_move(props),
                        ...
                        buffer buf) mutable {
                        auto& body = variant_get<buffer>(var);
                        props.emplace_back(v5::property::maximum_packet_size(
                            body.begin(), body.end()));
                        process_property_id(force_move(self),
                            force_move(session_life_keeper), force_move(buf),
                            rest, force_move(props), force_move(handler));
            ...
            case v5::property::id::wildcard_subscription_available: {
                ...
                process_nbytes(force_move(self),
                    ...
                    [this, props = force_move(props),
                        ...
                        auto& body = variant_get<buffer>(var);
                        props.emplace_back(
                            v5::property::wildcard_subscription_available(
                                body.begin(), body.end()));
                        process_property_id(force_move(self),
                            force_move(session_life_keeper), force_move(buf),
                            rest, force_move(props), force_move(handler));
            ...
            case v5::property::id::subscription_identifier_available: {
                ...
                process_nbytes(force_move(self),
                    ...
                    [this, props = force_move(props),
                        ...
                        auto& body = variant_get<buffer>(var);
                        props.emplace_back(
                            v5::property::subscription_identifier_available(
                                body.begin(), body.end()));
                        process_property_id(force_move(self),
                            force_move(session_life_keeper), force_move(buf),
                            rest, force_move(props), force_move(handler));
            ...
            case v5::property::id::shared_subscription_available: {
                ...
                process_nbytes(force_move(self),
                    ...
                    [this, props = force_move(props),
                        ...
                        auto& body = variant_get<buffer>(var);
                        props.emplace_back(
                            v5::property::shared_subscription_available(
                                body.begin(), body.end()));
                        process_property_id(force_move(self),
                            force_move(session_life_keeper), force_move(buf),
                            rest, force_move(props), force_move(handler));
    ...
    void process_header(this_type_sp&& self, any&& session_life_keeper,
        bool all_read, std::size_t header_len, parse_handler&& handler) {
        if (all_read) {
            auto spa = make_shared_ptr_array(remaining_length_);
            auto ptr = spa.get();
            socket_->async_read(as::buffer(ptr, remaining_length_),
                [this, self = force_move(self),
                    session_life_keeper = force_move(session_life_keeper),
                    buf = buffer(
                        string_view(ptr, remaining_length_), force_move(spa)),
                    handler = force_move(handler)](
                    error_code ec, std::size_t bytes_transferred) mutable {
                    this->total_bytes_received_ += bytes_transferred;
                    if (!check_error_and_transferred_length(
                            ec, bytes_transferred, remaining_length_))
                        return;
                    handler(force_move(self), force_move(session_life_keeper),
                        force_move(buf), buffer());
                });
        ...
        if (header_len == 0) {
            force_move(handler)(force_move(self),
                force_move(session_life_keeper), buffer(), buffer());
            return;
        }

        socket_->async_read(as::buffer(buf_.data(), header_len),
            [this, self = force_move(self),
                session_life_keeper = force_move(session_life_keeper),
                header_len, handler = force_move(handler)](
                error_code ec, std::size_t bytes_transferred) mutable {
                this->total_bytes_received_ += bytes_transferred;
                if (!check_error_and_transferred_length(
                        ec, bytes_transferred, header_len))
                    return;
                handler(force_move(self), force_move(session_life_keeper),
                    buffer(string_view(buf_.data(), header_len)), buffer());
            });
    }

    // process connect

    struct process_connect : as::coroutine,
                             std::enable_shared_from_this<process_connect> {
        ...
        process_connect(ep_t& ep, bool all_read)
            : ep_{ep}, all_read_{all_read} {}
        void operator()(ep_t_sp spep, any&& session_life_keeper,
            ...
            reenter(this) {
                ...
                // header
                yield ep_.process_header(force_move(spep),
                    std::make_tuple(force_move(session_life_keeper),
                        this->shared_from_this()),
                    all_read_, header_len_, [this](auto&&... args) {
                ...
                yield {
                    ...
                    static constexpr char protocol_name[] = {
                        0x00, 0x04, 'M', 'Q', 'T', 'T'};
                    if (std::memcmp(buf.data(), protocol_name,
                    ...
                    ep_.clean_start_ =
                        connect_flags::has_clean_start(connect_flag_);

                    buf.remove_prefix(header_len_);  // consume buffer
                    if (ep_.version_ == protocol_version::v5) {
                        // properties
                        ep_.process_properties(force_move(spep),
                            force_move(session_life_keeper), force_move(buf),
                            [this](auto&&... args) {
                                (*this)(std::forward<decltype(args)>(args)...);
                            });
                    } else {
                        (*this)(force_move(spep),
                            force_move(session_life_keeper), v5::properties(),
                            force_move(buf));
                    }
                }
                ...
                // client_id
                yield ep_.process_string(force_move(spep),
                    force_move(session_life_keeper), force_move(remain_buf),
                    [this](auto&&... args) {
                        (*this)(std::forward<decltype(args)>(args)...);
                    });
                ...
                if (connect_flags::has_will_flag(connect_flag_)) {
                    if (ep_.version_ == protocol_version::v5) {
                        // will properties
                        yield ep_.process_properties(force_move(spep),
                            force_move(session_life_keeper),
                            force_move(remain_buf), [this](auto&&... args) {
                                (*this)(std::forward<decltype(args)>(args)...);
                            });
                        will_props_ =
                    ...
                    // will payload
                    yield ep_.process_binary(force_move(spep),
                        force_move(session_life_keeper), force_move(remain_buf),
                        [this](auto&&... args) {
                            (*this)(std::forward<decltype(args)>(args)...);
                        });
                ...
                if (!ep_.set_values_from_props_on_connection(
                        connection_type::server, props_))
                    return;
                switch (ep_.version_) {
                    case protocol_version::v3_1_1:
                        if (ep_.on_connect(force_move(client_id_),
                                force_move(user_name_), force_move(password_),
                                connect_flags::has_will_flag(connect_flag_)
                                    ? optional<will>(in_place_init,
                                          force_move(will_topic_),
                                          force_move(will_payload_),
                                          connect_flags::has_will_retain(
                                              connect_flag_) |
                                              connect_flags::will_qos(
                                                  connect_flag_))
                                    : optional<will>(nullopt),
                                ep_.clean_session(), keep_alive_)) {
                            ep_.on_mqtt_message_processed(
                                force_move(std::get<0>(
                                    any_cast<std::tuple<any, process_type_sp>>(
                                        session_life_keeper))));
                        }
                        break;
                    case protocol_version::v5:
                        if (ep_.on_v5_connect(force_move(client_id_),
                                force_move(user_name_), force_move(password_),
                                connect_flags::has_will_flag(connect_flag_)
                                    ? optional<will>(in_place_init,
                                          force_move(will_topic_),
       ...
       private:
        static constexpr std::size_t header_len_ = 2 +  // string length
                                                   4 +  // "MQTT" string
                                                   1 +  // ProtocolVersion
                                                   1 +  // ConnectFlag
                                                   2;   // KeepAlive
    ...
    struct process_connack : as::coroutine,
                             std::enable_shared_from_this<process_connack> {
        using ep_t = this_type;
        using ep_t_sp = this_type_sp;
        using process_type = process_connack;
        using process_type_sp = std::shared_ptr<process_type>;

        process_connack(ep_t& ep, bool all_read)
            : ep_{ep}, all_read_{all_read} {}
        void operator()(ep_t_sp spep, any&& session_life_keeper,
            parse_handler_variant var = std::size_t(0),
            buffer /*remain_buf*/ = buffer()) {
            reenter(this) {
                ...
                yield {
                    ...
                    switch (ep_.version_) {
                        case protocol_version::v3_1_1:
                            reason_code_ =
                                static_cast<connect_return_code>(buf[1]);
                            break;
                        case protocol_version::v5:
                            reason_code_ =
                                static_cast<v5::connect_reason_code>(buf[1]);
                            break;
                        default:
                            BOOST_ASSERT(false);
                    }
                ...
                if (!ep_.set_values_from_props_on_connection(
                        connection_type::client, props_))
                    return;
                {
                    auto connack_proc = [this](
                                            any&& session_life_keeper) mutable {
                        switch (ep_.version_) {
                            case protocol_version::v3_1_1:
                                if (ep_.on_connack(session_present_,
                                        variant_get<connect_return_code>(
                                            reason_code_))) {
                                    ep_.on_mqtt_message_processed(
                                        force_move(std::get<0>(any_cast<
                                            std::tuple<any, process_type_sp>>(
                                            session_life_keeper))));
                                }
                                break;
                            case protocol_version::v5:
                                if (ep_.on_v5_connack(session_present_,
                                        variant_get<v5::connect_reason_code>(
                                            reason_code_),
                                        force_move(props_))) {
                                    ep_.on_mqtt_message_processed(
                                        force_move(std::get<0>(any_cast<
                    ...
                    // Note: boost:variant has no featue to query if the variant
                    // currently holds a specific type. MQTT_CPP could create a
                    // type traits function to match the provided type to the
                    // index in the boost::variant type list, but for now it
                    ...
                    if (((0 == variant_idx(reason_code_)) &&
                        ...
                        ((1 == variant_idx(reason_code_)) &&
                            (v5::connect_reason_code::success ==
                                variant_get<v5::connect_reason_code>(
                                    reason_code_)))) {
                        // If session_present is false, then call
                        // clear_session_data(). Here is the reason why it works
                        // well.
                        // ---
                        // https://docs.oasis-open.org/mqtt/mqtt/v5.0/os/mqtt-v5.0-os.html#_Toc3901078
                        //
                        // If the Server accepts a connection with Clean Start
                        // set to 1, the Server MUST set Session Present to 0 in
                        // the CONNACK packet in addition to setting a 0x00
                        ...
                        //
                        // The Client can avoid implementing its own Session
                        // expiry and instead rely on the Session Present flag
                        // returned from the Server to determine if the Session
                        // had expired. If the Client does implement its own
                        // Session expiry, it needs to store the time at which
                        // the Session State will be deleted as part of its
                        // Session State.
                        // ---
                        //
                        // Also it can  avoid the following client side and
                        // broker side session state mismatch autonatically.
                        //
                        // ---
                        // https://docs.oasis-open.org/mqtt/mqtt/v5.0/os/mqtt-v5.0-os.html#_Toc3901078
                        //
                        // If the Client does not have Session State and
                        // receives Session Present set to 1 it MUST close the
                        // Network Connection [MQTT-3.2.2-4]. If it wishes to
                        // restart with a new Session the Client can reconnect
                        // using Clean Start set to 1. If the Client does have
                        // Session State and receives Session Present set to 0
                        // it MUST discard its Session State if it continues
                        // with the Network Connection [MQTT-3.2.2-5].
                        // ---
                        if (session_present_) {
                            if (ep_.async_operation_) {
                                // Until all stored messages are written to
                                // internal send buffer, disable further async
                                // reading of incoming packets..
                                ep_.async_read_on_message_processed_ = false;
                                auto async_connack_proc =
                                    [this, spep = force_move(spep),
                                        session_life_keeper =
                                            force_move(session_life_keeper),
                                        connack_proc = force_move(
                                            connack_proc)]() mutable {
                                        // All stored messages are sent, so
                                        // re-enable reading of incoming
                                        // packets. and notify the end user code
                                        // that the connack packet was received.
                                        ep_.async_read_on_message_processed_ =
                                            true;
                                        connack_proc(
                                            force_move(session_life_keeper));
                                    };
                                ep_.async_send_store(
    ...
    struct process_publish : as::coroutine,
                             std::enable_shared_from_this<process_publish> {
        using ep_t = this_type;
        using ep_t_sp = this_type_sp;
        using process_type = process_publish;
        using process_type_sp = std::shared_ptr<process_type>;

        process_publish(ep_t& ep, bool all_read)
            : ep_{ep}, all_read_{all_read} {}
        void operator()(ep_t_sp spep, any&& session_life_keeper,
            parse_handler_variant var = std::size_t(0),
            buffer remain_buf = buffer()) {
            reenter(this) {
                ...
                if (qos_value_ == qos::at_least_once ||
                    qos_value_ == qos::exactly_once) {
                    yield ep_.process_packet_id(force_move(spep),
                        force_move(session_life_keeper), force_move(remain_buf),
                        [this](auto&&... args) {
                            (*this)(std::forward<decltype(args)>(args)...);
                        });
                ...
                yield ep_.process_nbytes(force_move(spep),
                    force_move(session_life_keeper), force_move(remain_buf),
                    ep_.remaining_length_, [this](auto&&... args) {
                        (*this)(std::forward<decltype(args)>(args)...);
                    });
                {
                    auto handler_call = [&] {
                        auto check_full = [&] {
                            LockGuard<Mutex> lck(ep_.publish_received_mtx_);
                            if (ep_.publish_received_.size() ==
                                ep_.publish_recv_max_) {
                                return true;
                            }
                        ...
                        switch (ep_.version_) {
                            case protocol_version::v3_1_1:
                                return ep_.on_publish(packet_id_,
                            ...
                            case protocol_version::v5:
                                switch (qos_value_) {
                                    case qos::at_most_once:
                                        break;
                                        // automatically response error using
                                        // puback / pubrec but the connection
                                        // continues. publish handler is not
                                        // called
                                        ...
                                        if (check_full()) {
                                            if (ep_.async_operation_) {
                                                ep_.async_send_puback(
                                                    *packet_id_,
                                                    v5::puback_reason_code::
                                                        quota_exceeded,
                                                    v5::properties{},
                                                    [](auto) {});
                                            } else {
                                                ep_.send_puback(*packet_id_,
                                                    v5::puback_reason_code::
                                                        quota_exceeded,
                                                    v5::properties{});
                                            }
                                ...
                                if (topic_name_.empty()) {
                                    if (auto topic_alias =
                                            get_topic_alias_from_props(
                                                props_)) {
                                        if (topic_alias.value() == 0 ||
                                            topic_alias.value() >
                                                ep_.topic_alias_recv_.value()
                                                    .max()) {
                                            ep_.send_error_disconnect(
                                                v5::disconnect_reason_code::
                                                    topic_alias_invalid);
                                            ep_.call_protocol_error_handlers();
                                            return false;
                                        }
                                        auto topic_name = [&] {
                                            LockGuard<Mutex> lck(
                                                ep_.topic_alias_recv_mtx_);
                                            if (ep_.topic_alias_recv_) {
                                                return ep_.topic_alias_recv_
                                                    .value()
                                                    .find(topic_alias.value());
                                            }
                                            return std::string();
                                        }();
                                        if (topic_name.empty()) {
                                            ...
                                            topic_name_ =
                                                allocate_buffer(topic_name);
                                        }
                                    }
                                } else {
                                    if (auto topic_alias =
                                            ep_.get_topic_alias_from_props(
                                                props_)) {
                                        LockGuard<Mutex> lck(
                                            ep_.topic_alias_recv_mtx_);
                                        if (ep_.topic_alias_recv_) {
                                            ep_.topic_alias_recv_.value()
                                                .insert_or_update(topic_name_,
                                                    topic_alias.value());
                                        }
                                    }
                                }
                                {
                                    auto ret = ep_.on_v5_publish(packet_id_,
                                        publish_options(ep_.fixed_header_),
                                        force_move(topic_name_),
                                        force_move(variant_get<buffer>(var)),
                                        force_move(props_));
                        ...
                        case qos::at_least_once:
                            if (handler_call()) {
                                ...
                                ep_.auto_pub_response(
                                    ...
                                    [this] {
                                        if (ep_.connected_) {
                                            ep_.async_send_puback(*packet_id_,
                                                v5::puback_reason_code::success,
                                                v5::properties{}, [](auto) {});
                                        }
                                    });
                            ...
                            break;
                        case qos::exactly_once:
                            if (ep_.qos2_publish_handled_.find(*packet_id_) ==
                                ep_.qos2_publish_handled_.end()) {
                                if (handler_call()) {
                                    ep_.on_mqtt_message_processed(
                                        force_move(std::get<0>(any_cast<
                                            std::tuple<any, process_type_sp>>(
                                            session_life_keeper))));
                                    ep_.qos2_publish_handled_.emplace(
                                        *packet_id_);
                                    ep_.auto_pub_response(
                                        [this] {
                                            if (ep_.connected_) {
                                ...
                                ep_.on_mqtt_message_processed(
                                    force_move(std::get<0>(any_cast<
                                        ...
                                        session_life_keeper))));
                                if (ep_.async_operation_) {
                                    ep_.async_send_pubrec(*packet_id_,
    ...
    struct process_puback : as::coroutine,
                            std::enable_shared_from_this<process_puback> {
        using ep_t = this_type;
        using ep_t_sp = this_type_sp;
        using process_type = process_puback;
        using process_type_sp = std::shared_ptr<process_type>;

        process_puback(ep_t& ep, bool all_read)
            : ep_{ep}, all_read_{all_read} {}
        void operator()(ep_t_sp spep, any&& session_life_keeper,
            parse_handler_variant var = std::size_t(0),
            buffer remain_buf = buffer()) {
            reenter(this) {
                ...
                if (ep_.remaining_length_ == 0) {
                    ...
                    // If the Remaining Length is 0, there is no reason code &
                    // property length the value of success is used for reason
                    // code, the value of 0 is used for property length
                    ...
                    reason_code_ = static_cast<v5::puback_reason_code>(
                        variant_get<buffer>(var)[0]);

                    // https://docs.oasis-open.org/mqtt/mqtt/v5.0/os/mqtt-v5.0-os.html#_Toc3901126
                    // If the Remaining Length is 0, there is no property length
                ...
                auto erased = [&] {
                    ...
                    auto r = idx.equal_range(std::make_tuple(
                        packet_id_, control_packet_type::puback));
                ...
                switch (ep_.version_) {
                    ...
                    case protocol_version::v5:
                        ...
                        if (ep_.on_v5_puback(
                                packet_id_, reason_code_, force_move(props_))) {
                            ep_.on_mqtt_message_processed(
                                force_move(std::get<0>(
                                    any_cast<std::tuple<any, process_type_sp>>(
                                        session_life_keeper))));
    ...
    struct process_pubrec : as::coroutine,
                            std::enable_shared_from_this<process_pubrec> {
        using ep_t = this_type;
        using ep_t_sp = this_type_sp;
        using process_type = process_pubrec;
        using process_type_sp = std::shared_ptr<process_type>;

        process_pubrec(ep_t& ep, bool all_read)
            : ep_{ep}, all_read_{all_read} {}
        void operator()(ep_t_sp spep, any&& session_life_keeper,
            parse_handler_variant var = std::size_t(0),
            buffer remain_buf = buffer()) {
            reenter(this) {
                ...
                if (ep_.remaining_length_ == 0) {
                    ...
                    reason_code_ = static_cast<v5::pubrec_reason_code>(
                        variant_get<buffer>(var)[0]);
                ...
                auto erased = [&] {
                    ...
                    auto& idx = ep_.store_.template get<tag_packet_id_type>();
                    auto r = idx.equal_range(std::make_tuple(
                        packet_id_, control_packet_type::pubrec));

                    // pubrec packet_id is not matched to publish
                    if (std::get<0>(r) == std::get<1>(r)) return false;

                    idx.erase(std::get<0>(r), std::get<1>(r));
                    // packet_id should be erased here only if reason_code is
                    // error. Otherwise the packet_id is continue to be used for
                    // pubrel/pubcomp.
                    if (is_error(reason_code_))
                        ep_.pid_man_.release_id(packet_id_);
                    return true;
                }();
                {
                    auto res = [&] {
                        auto rc = [&] {
                            ...
                            return v5::pubrel_reason_code::
                                packet_identifier_not_found;
                        }();
                        ep_.auto_pub_response(
                            [&] {
                                if (ep_.connected_) {
                                    ep_.send_pubrel(packet_id_, rc,
                                        v5::properties{}, any{});
                                } else {
                                    ep_.store_pubrel(packet_id_, rc,
                                        v5::properties{}, any{});
                                }
                            },
                            [&] {
                                if (ep_.connected_) {
                                    ep_.async_send_pubrel(packet_id_, rc,
                                        v5::properties{}, any{}, [](auto) {});
                                } else {
                                    ep_.store_pubrel(packet_id_, rc,
                                        v5::properties{}, any{});
                                }
                    ...
                    switch (ep_.version_) {
                        ...
                        case protocol_version::v5:
                            ...
                            if (ep_.on_v5_pubrec(packet_id_, reason_code_,
                                    force_move(props_))) {
    ...
    struct process_pubrel : as::coroutine,
                            std::enable_shared_from_this<process_pubrel> {
        using ep_t = this_type;
        using ep_t_sp = this_type_sp;
        using process_type = process_pubrel;
        using process_type_sp = std::shared_ptr<process_type>;

        process_pubrel(ep_t& ep, bool all_read)
            : ep_{ep}, all_read_{all_read} {}
        void operator()(ep_t_sp spep, any&& session_life_keeper,
            parse_handler_variant var = std::size_t(0),
            buffer remain_buf = buffer()) {
            reenter(this) {
                ...
                if (ep_.remaining_length_ == 0) {
                    ...
                    reason_code_ = static_cast<v5::pubrel_reason_code>(
                        variant_get<buffer>(var)[0]);
...

                {
                    auto res = [&] {
                        ep_.auto_pub_response(
                            [&] {
                                if (ep_.connected_) {
                                    ep_.send_pubcomp(packet_id_,
                                        static_cast<v5::pubcomp_reason_code>(
                                            reason_code_),
                                        v5::properties{});
                                }
                            },
                            [&] {
                                if (ep_.connected_) {
                                    ep_.async_send_pubcomp(packet_id_,
                                        static_cast<v5::pubcomp_reason_code>(
                                            reason_code_),
                                        v5::properties{}, [](auto) {});
                                }
                    ...
                    switch (ep_.version_) {
                        case protocol_version::v3_1_1:
                            ...
                            break;
                        case protocol_version::v5:
                            if (ep_.on_v5_pubrel(packet_id_, reason_code_,
                                    force_move(props_))) {
                                res();
                                ep_.on_mqtt_message_processed(
                                    force_move(std::get<0>(any_cast<
    ...
    struct process_pubcomp : as::coroutine,
                             std::enable_shared_from_this<process_pubcomp> {
        using ep_t = this_type;
        using ep_t_sp = this_type_sp;
        using process_type = process_pubcomp;
        using process_type_sp = std::shared_ptr<process_type>;

        process_pubcomp(ep_t& ep, bool all_read)
            : ep_{ep}, all_read_{all_read} {}
        void operator()(ep_t_sp spep, any&& session_life_keeper,
            parse_handler_variant var = std::size_t(0),
            buffer remain_buf = buffer()) {
            reenter(this) {
                ...
                if (ep_.remaining_length_ == 0) {
                    ...
                    reason_code_ = static_cast<v5::pubcomp_reason_code>(
                        variant_get<buffer>(var)[0]);
                ...
                auto erased = [&] {
                    ...
                    auto& idx = ep_.store_.template get<tag_packet_id_type>();
                    auto r = idx.equal_range(std::make_tuple(
                        packet_id_, control_packet_type::pubcomp));
                ...
                switch (ep_.version_) {
                    case protocol_version::v3_1_1:
                        ...
                        break;
                    case protocol_version::v5:
                        if (erased && ep_.resend_pubrel_.find(packet_id_) ==
                                          ep_.resend_pubrel_.end()) {
                            ep_.send_publish_queue_one();
                        }
                        if (ep_.on_v5_pubcomp(
                                packet_id_, reason_code_, force_move(props_))) {
                            ep_.on_mqtt_message_processed(
                                force_move(std::get<0>(
                                    any_cast<std::tuple<any, process_type_sp>>(
    ...
    struct process_subscribe : as::coroutine,
                               std::enable_shared_from_this<process_subscribe> {
        using ep_t = this_type;
        using ep_t_sp = this_type_sp;
        using process_type = process_subscribe;
        using process_type_sp = std::shared_ptr<process_type>;

        process_subscribe(ep_t& ep, bool all_read)
            : ep_{ep}, all_read_{all_read} {}
        void operator()(ep_t_sp spep, any&& session_life_keeper,
            parse_handler_variant var = std::size_t(0),
            buffer remain_buf = buffer()) {
            reenter(this) {
                ...
                while (true) {
                    ...
                    sn_tf_opt_ =
                        parse_shared_subscription(variant_get<buffer>(var));
                    if (!sn_tf_opt_) {
                    ...
                    sub_opts_opt_.emplace(
                        static_cast<std::uint8_t>(variant_get<buffer>(var)[0]));
                    ...
                    if (ep_.remaining_length_ == 0) {
                        switch (ep_.version_) {
                            case protocol_version::v3_1_1:
                                if (ep_.on_subscribe(
                                        packet_id_, force_move(entries_))) {
                                    ep_.on_mqtt_message_processed(
                                        force_move(std::get<0>(any_cast<
                                            std::tuple<any, process_type_sp>>(
                                            session_life_keeper))));
                                ...
                                break;
                            case protocol_version::v5:
                                if (ep_.on_v5_subscribe(packet_id_,
                                        force_move(entries_),
                                        force_move(props_))) {
                                    ep_.on_mqtt_message_processed(
                                        force_move(std::get<0>(any_cast<
    ...
    struct process_suback : as::coroutine,
                            std::enable_shared_from_this<process_suback> {
        using ep_t = this_type;
        using ep_t_sp = this_type_sp;
        using process_type = process_suback;
        using process_type_sp = std::shared_ptr<process_type>;

        process_suback(ep_t& ep, bool all_read)
            : ep_{ep}, all_read_{all_read} {}
        void operator()(ep_t_sp spep, any&& session_life_keeper,
            parse_handler_variant var = std::size_t(0),
            buffer remain_buf = buffer()) {
            reenter(this) {
                ...
                switch (ep_.version_) {
                    case protocol_version::v3_1_1: {
                        // TODO: We can avoid an allocation by casting the raw
                        // bytes of the MQTT_NS::buffer that is being parsed,
                        // and instead call the suback handler with an std::span
                        // and the MQTT_NS::buffer (as lifekeeper)
                        ...
                        std::transform(body.begin(), body.end(),
                            ...
                            // The SUBACK Packet sent by the Server to the
                            // Client MUST contain a return code for each Topic
                            // Filter/QoS pair. This return code MUST either
                            // show the maximum QoS that was granted for that
                    ...
                    case protocol_version::v5: {
                        ...
                        std::transform(body.begin(), body.end(),
                            ...
                            // The SUBACK packet sent by the Server to the
                            // Client MUST contain a Reason Code for each Topic
                            // Filter/Subscription Option pair [MQTT-3.8.4-6].
                            // This Reason Code MUST either show the maximum QoS
                            // that was granted for that Subscription or
                            // indicate that the subscription failed
                            // [MQTT-3.8.4-7].
                            [&](auto const& e) {
                                return static_cast<v5::suback_reason_code>(e);
                            });
                        if (ep_.on_v5_suback(packet_id_, force_move(reasons),
                                force_move(props_))) {
                            ep_.on_mqtt_message_processed(
                                force_move(std::get<0>(
                                    any_cast<std::tuple<any, process_type_sp>>(
    ...
    struct process_unsubscribe
        : as::coroutine,
          std::enable_shared_from_this<process_unsubscribe> {
        using ep_t = this_type;
        using ep_t_sp = this_type_sp;
        using process_type = process_unsubscribe;
        using process_type_sp = std::shared_ptr<process_type>;

        process_unsubscribe(ep_t& ep, bool all_read)
            : ep_{ep}, all_read_{all_read} {}
        void operator()(ep_t_sp spep, any&& session_life_keeper,
            parse_handler_variant var = std::size_t(0),
            buffer remain_buf = buffer()) {
                    ...
                    if (ep_.remaining_length_ == 0) {
                        switch (ep_.version_) {
                            case protocol_version::v3_1_1:
                                ...
                                break;
                            case protocol_version::v5:
                                if (ep_.on_v5_unsubscribe(packet_id_,
                                        force_move(entries_),
                                        force_move(props_))) {
                                    ep_.on_mqtt_message_processed(
                                        force_move(std::get<0>(any_cast<
    ...
    struct process_unsuback : as::coroutine,
                              std::enable_shared_from_this<process_unsuback> {
        using ep_t = this_type;
        using ep_t_sp = this_type_sp;
        using process_type = process_unsuback;
        using process_type_sp = std::shared_ptr<process_type>;

        process_unsuback(ep_t& ep, bool all_read)
            : ep_{ep}, all_read_{all_read} {}
        void operator()(ep_t_sp spep, any&& session_life_keeper,
            parse_handler_variant var = std::size_t(0),
            buffer remain_buf = buffer()) {
            reenter(this) {
                ...
                if (ep_.version_ == protocol_version::v5) {
                    ...
                    std::transform(body.begin(), body.end(), reasons_.begin(),
                        [&](auto const& e) {
                ...
                switch (ep_.version_) {
                    case protocol_version::v3_1_1:
                        ...
                        break;
                    case protocol_version::v5:
                        if (ep_.on_v5_unsuback(packet_id_, force_move(reasons_),
                                force_move(props_))) {
                            ep_.on_mqtt_message_processed(
                                force_move(std::get<0>(
                                    any_cast<std::tuple<any, process_type_sp>>(
    ...
    void process_pingreq(any session_life_keeper) {
    ...
    void process_pingresp(any session_life_keeper) {
        static constexpr std::size_t header_len = 0;
    ...
    struct process_disconnect
        : as::coroutine,
          std::enable_shared_from_this<process_disconnect> {
        using ep_t = this_type;
        using ep_t_sp = this_type_sp;
        using process_type = process_disconnect;
        using process_type_sp = std::shared_ptr<process_type>;

        process_disconnect(ep_t& ep, bool all_read)
            : ep_{ep}, all_read_{all_read} {}
        void operator()(ep_t_sp spep, any&& session_life_keeper,
            parse_handler_variant var = std::size_t(0),
            buffer remain_buf = buffer()) {
            reenter(this) {
                if (ep_.remaining_length_ > 0) {
                    ...
                    yield ep_.process_nbytes(force_move(spep),
                        force_move(session_life_keeper),
                        force_move(variant_get<buffer>(var)), 1,
                        [this](auto&&... args) {
                            (*this)(std::forward<decltype(args)>(args)...);
                        });
                    reason_code_ = static_cast<v5::disconnect_reason_code>(
                        variant_get<buffer>(var)[0]);
                    ...
                    switch (ep_.version_) {
                        case protocol_version::v3_1_1:
                            ...
                            break;
                        case protocol_version::v5:
                            ep_.on_v5_disconnect(
                                reason_code_, force_move(props_));
                            break;
                        default:
                            BOOST_ASSERT(false);
                    }
                    ...
                    ep_.on_mqtt_message_processed(force_move(
                        std::get<0>(any_cast<std::tuple<any, process_type_sp>>(
                            session_life_keeper))));
                    return;
                }
                switch (ep_.version_) {
                ...
                MQTT_LOG("mqtt_impl", trace)
                    ...
                    << "receive DISCONNECT call shutdown";
                ep_.shutdown(*ep_.socket_);
                ep_.on_mqtt_message_processed(force_move(session_life_keeper));
            }
        }
    ...
    struct process_auth : as::coroutine,
                          std::enable_shared_from_this<process_auth> {
        using ep_t = this_type;
        using ep_t_sp = this_type_sp;
        using process_type = process_auth;
        using process_type_sp = std::shared_ptr<process_type>;

        process_auth(ep_t& ep, bool all_read) : ep_{ep}, all_read_{all_read} {}
        void operator()(ep_t_sp spep, any&& session_life_keeper,
            parse_handler_variant var = std::size_t(0),
            buffer remain_buf = buffer()) {
            reenter(this) {
                ...
                if (ep_.remaining_length_ > 0) {
                    ...
                    reason_code_ = static_cast<v5::auth_reason_code>(
                        variant_get<buffer>(var)[0]);
                    ...
                    if (ep_.on_v5_auth(reason_code_, force_move(props_))) {
                        ep_.on_mqtt_message_processed(force_move(std::get<0>(
                            any_cast<std::tuple<any, process_type_sp>>(
                                session_life_keeper))));
                    }
                    return;
    ...
    // Blocking senders.
    void send_connect(buffer client_id, optional<buffer> user_name,
        optional<buffer> password, optional<will> w,
        std::uint16_t keep_alive_sec, v5::properties props) {
        shutdown_requested_ = false;
        switch (version_) {
            case protocol_version::v3_1_1:
                do_sync_write(v3_1_1::connect_message(keep_alive_sec,
                    force_move(client_id), clean_session(), force_move(w),
                    force_move(user_name), force_move(password)));
                break;
            case protocol_version::v5:
                update_values_and_props_on_start_connection(props);
                do_sync_write(
                    v5::connect_message(keep_alive_sec, force_move(client_id),
                        clean_start(), force_move(w), force_move(user_name),
                        force_move(password), force_move(props)));
                break;
            default:
                BOOST_ASSERT(false);
                break;
    ...
    void send_connack(bool session_present,
        variant<connect_return_code, v5::connect_reason_code> reason_code,
        v5::properties props) {
        switch (version_) {
            case protocol_version::v3_1_1: {
                auto msg = v3_1_1::connack_message(session_present,
            ...
            case protocol_version::v5: {
                update_values_and_props_on_start_connection(props);
                auto msg = v5::connack_message(session_present,
    ...
    std::enable_if_t<std::is_same<PublishMessage,
        v5::basic_publish_message<PacketIdBytes>>::value>
    remove_topic_alias(PublishMessage& msg, any& life_keeper) {
        if (msg.topic().empty()) {
            ...
            if (auto ta_opt = get_topic_alias_from_props(msg.props())) {
                ...
                if (topic_alias_send_)
                    t = topic_alias_send_.value().find(ta_opt.value());
                if (t.empty()) {
                    MQTT_LOG("mqtt_impl", error)
                        << MQTT_ADD_VALUE(address, this)
                        << "publish topic_name is empty, topic alias "
                        << ta_opt.value() << " is not registered.";
                    throw protocol_error();
                } else {
                    MQTT_LOG("mqtt_impl", trace)
                        << MQTT_ADD_VALUE(address, this)
                        << "topia alias : " << t << " - " << ta_opt.value()
                        << " is recovered for store.";
                    auto topic_name_buf = allocate_buffer(t);
                    msg.set_topic_name(as::buffer(topic_name_buf));
                    life_keeper = std::make_tuple(
                        force_move(life_keeper), force_move(topic_name_buf));
                }
            } else {
                MQTT_LOG("mqtt_impl", error)
                    << MQTT_ADD_VALUE(address, this)
    ...
    apply_topic_alias(PublishMessage msg, any life_keeper) {
        auto clear_topic_name_and_add_topic_alias = [&](topic_alias_t ta) {
        ...
        if (msg.topic().empty()) {
            ...
            if (auto ta_opt = get_topic_alias_from_props(msg.props())) {
                MQTT_LOG("mqtt_impl", trace)
                    ...
                    << ta_opt.value() << " is registered.";
                LockGuard<Mutex> lck(topic_alias_send_mtx_);
                if (topic_alias_send_) {
                    topic_alias_send_.value().insert_or_update(
                        msg.topic(), ta_opt.value());
                }
                ...
                if (topic_alias_send_) {
                    ...
                    if (auto ta_opt =
                            topic_alias_send_.value().find(msg.topic())) {
                        MQTT_LOG("mqtt_impl", trace)
                            << MQTT_ADD_VALUE(address, this)
                            << "topia alias : " << msg.topic() << " - "
                            << ta_opt.value() << " is found.";
                        topic_alias_send_.value().insert_or_update(
                            msg.topic(), ta_opt.value());  // update ts
                        clear_topic_name_and_add_topic_alias(ta_opt.value());
                    } else {
                        topic_alias_send_.value().insert_or_update(
                            msg.topic(), lru_ta);  // remap topic alias
    ...
    template <typename PublishMessage, typename SerializePublish,
        typename ReceiveMaximumProc>
    bool preprocess_publish_message(PublishMessage const& msg, any life_keeper,
        SerializePublish&& serialize_publish,
        ReceiveMaximumProc&& receive_maximum_proc, bool register_pid = false) {
        ...
        if (qos_value == qos::at_least_once || qos_value == qos::exactly_once) {
            ...
            store_.emplace(packet_id,
                qos_value == qos::at_least_once ? control_packet_type::puback
                                                : control_packet_type::pubrec,
                store_msg, force_move(life_keeper));
            (this->*std::forward<decltype(serialize_publish)>(
                        serialize_publish))(store_msg);
            return std::forward<decltype(receive_maximum_proc)>(
                receive_maximum_proc)(force_move(store_msg));
        }
        return true;
    }
    ...
    typename std::enable_if<
        as::is_const_buffer_sequence<ConstBufferSequence>::value>::type
    send_publish(packet_id_t packet_id, as::const_buffer topic_name,
        ConstBufferSequence payloads, publish_options pubopts,
        v5::properties props, any life_keeper) {
        auto do_send_publish = [&](auto msg, auto&& serialize_publish,
                                   auto&& receive_maximum_proc) {
            auto msg_lk = apply_topic_alias(msg, life_keeper);
            if (maximum_packet_size_send_ <
                size<PacketIdBytes>(std::get<0>(msg_lk))) {
            ...
            if (preprocess_publish_message(msg, life_keeper,
                    std::forward<decltype(serialize_publish)>(
                        serialize_publish),
                    std::forward<decltype(receive_maximum_proc)>(
                        receive_maximum_proc))) {
                do_sync_write(force_move(std::get<0>(msg_lk)));
            ...
            case protocol_version::v3_1_1:
                do_send_publish(
                    v3_1_1::basic_publish_message<PacketIdBytes>(
                        packet_id, topic_name, force_move(payloads), pubopts),
                    &endpoint::on_serialize_publish_message,
                    [](auto&&) { return true; });
                break;
            case protocol_version::v5:
                do_send_publish(v5::basic_publish_message<PacketIdBytes>(
                                    packet_id, topic_name, force_move(payloads),
                                    pubopts, force_move(props)),
                    &endpoint::on_serialize_v5_publish_message,
                    [this](v5::basic_publish_message<PacketIdBytes>&& msg) {
    ...
    void send_puback(packet_id_t packet_id, v5::puback_reason_code reason,
        v5::properties props) {
        switch (version_) {
            case protocol_version::v3_1_1: {
                auto msg =
            ...
            case protocol_version::v5: {
                auto msg = v5::basic_puback_message<PacketIdBytes>(
                    packet_id, reason, force_move(props));
                if (maximum_packet_size_send_ < size<PacketIdBytes>(msg)) {
                    throw packet_size_error();
                }
                do_sync_write(force_move(msg));
    ...
    void send_pubrec(packet_id_t packet_id, v5::pubrec_reason_code reason,
        v5::properties props) {
        switch (version_) {
            case protocol_version::v3_1_1: {
                auto msg =
            ...
            case protocol_version::v5: {
                auto msg = v5::basic_pubrec_message<PacketIdBytes>(
                    packet_id, reason, force_move(props));
                if (maximum_packet_size_send_ < size<PacketIdBytes>(msg)) {
                    throw packet_size_error();
                }
    ...
    void send_pubrel(packet_id_t packet_id, v5::pubrel_reason_code reason,
        v5::properties props, any life_keeper) {
        auto impl = [&](auto msg, auto const& serialize) {
            {
                if (maximum_packet_size_send_ < size<PacketIdBytes>(msg)) {
                    throw packet_size_error();
                }
                ...
                auto ret = store_.emplace(packet_id,
                    control_packet_type::pubcomp, msg, force_move(life_keeper));
                ...
                // then send corresponding pubrel twice is a possible
                // client/server implementation. In this case, overwrite store_.
                if (!ret.second) {
                    MQTT_LOG("mqtt_impl", warning)
                        << MQTT_ADD_VALUE(address, this) << "overwrite pubrel"
                        << " packet_id:" << packet_id;
                    store_.modify(ret.first, [&](auto& e) {
                        e = store(packet_id, control_packet_type::pubcomp, msg,
                            life_keeper);
                    });
                }
            }
        ...
        switch (version_) {
            case protocol_version::v3_1_1:
                impl(v3_1_1::basic_pubrel_message<PacketIdBytes>(packet_id),
                    &endpoint::on_serialize_pubrel_message);
                break;
            case protocol_version::v5:
                impl(v5::basic_pubrel_message<PacketIdBytes>(
                         packet_id, reason, force_move(props)),
                    &endpoint::on_serialize_v5_pubrel_message);
                break;
            default:
    ...
    void store_pubrel(packet_id_t packet_id, v5::pubrel_reason_code reason,
        v5::properties props, any life_keeper) {
        auto impl = [&](auto msg, auto const& serialize) {
            {
                LockGuard<Mutex> lck(store_mtx_);
    ...
    void send_pubcomp(packet_id_t packet_id, v5::pubcomp_reason_code reason,
        v5::properties props) {
        switch (version_) {
            case protocol_version::v3_1_1: {
                auto msg =
            ...
            case protocol_version::v5: {
                auto msg = v5::basic_pubcomp_message<PacketIdBytes>(
                    packet_id, reason, force_move(props));
                if (maximum_packet_size_send_ < size<PacketIdBytes>(msg)) {
                    throw packet_size_error();
                }
    ...
    void send_subscribe(
        std::vector<std::tuple<as::const_buffer, subscribe_options>> params,
        packet_id_t packet_id, v5::properties props) {
        for (auto const& p : params) {
            (void)p;
            BOOST_ASSERT(std::get<1>(p).get_qos() == qos::at_most_once ||
                         std::get<1>(p).get_qos() == qos::at_least_once ||
                         std::get<1>(p).get_qos() == qos::exactly_once);
        }
        switch (version_) {
            case protocol_version::v3_1_1: {
                auto msg = v3_1_1::basic_subscribe_message<PacketIdBytes>(
            ...
            case protocol_version::v5: {
                auto msg = v5::basic_subscribe_message<PacketIdBytes>(
                    force_move(params), packet_id, force_move(props));
                if (maximum_packet_size_send_ < size<PacketIdBytes>(msg)) {
                    {
                        LockGuard<Mutex> lck_store(store_mtx_);
                        pid_man_.release_id(packet_id);
    ...
    void send_suback(variant<std::vector<suback_return_code>,
                         std::vector<v5::suback_reason_code>>
                         params,
        packet_id_t packet_id, v5::properties props) {
        switch (version_) {
            case protocol_version::v3_1_1: {
                auto msg = v3_1_1::basic_suback_message<PacketIdBytes>(
                    force_move(
                        variant_get<std::vector<suback_return_code>>(params)),
                    packet_id);
                if (maximum_packet_size_send_ < size<PacketIdBytes>(msg)) {
                    throw packet_size_error();
                }
            ...
            case protocol_version::v5: {
                auto msg = v5::basic_suback_message<PacketIdBytes>(
                    force_move(variant_get<std::vector<v5::suback_reason_code>>(
                        params)),
                    packet_id, force_move(props));
                if (maximum_packet_size_send_ < size<PacketIdBytes>(msg)) {
                    throw packet_size_error();
    ...
    void send_unsubscribe(std::vector<as::const_buffer> params,
        packet_id_t packet_id, v5::properties props) {
        switch (version_) {
            case protocol_version::v3_1_1: {
                auto msg = v3_1_1::basic_unsubscribe_message<PacketIdBytes>(
                    force_move(params), packet_id);
                if (maximum_packet_size_send_ < size<PacketIdBytes>(msg)) {
                    {
                        LockGuard<Mutex> lck_store(store_mtx_);
            ...
            case protocol_version::v5: {
                auto msg = v5::basic_unsubscribe_message<PacketIdBytes>(
                    force_move(params), packet_id, force_move(props));
                if (maximum_packet_size_send_ < size<PacketIdBytes>(msg)) {
                    {
                        LockGuard<Mutex> lck_store(store_mtx_);
    ...
    void send_unsuback(packet_id_t packet_id) {
        switch (version_) {
            case protocol_version::v3_1_1: {
                auto msg =
                    v3_1_1::basic_unsuback_message<PacketIdBytes>(packet_id);
                if (maximum_packet_size_send_ < size<PacketIdBytes>(msg)) {
                    throw packet_size_error();
                }
                do_sync_write(force_move(msg));
    ...
    void send_unsuback(std::vector<v5::unsuback_reason_code> params,
        packet_id_t packet_id, v5::properties props) {
        switch (version_) {
            case protocol_version::v3_1_1:
                BOOST_ASSERT(false);
                break;
            case protocol_version::v5: {
                auto msg = v5::basic_unsuback_message<PacketIdBytes>(
                    force_move(params), packet_id, force_move(props));
                if (maximum_packet_size_send_ < size<PacketIdBytes>(msg)) {
                    throw packet_size_error();
                }
    ...
    void send_auth(v5::auth_reason_code reason, v5::properties props) {
        switch (version_) {
            case protocol_version::v3_1_1:
                BOOST_ASSERT(false);
                break;
    ...
    void send_disconnect(
        v5::disconnect_reason_code reason, v5::properties props) {
        switch (version_) {
            case protocol_version::v3_1_1: {
            ...
            MQTT_NS::visit(
                make_lambda_visitor(
                    [&](v3_1_1::basic_publish_message<PacketIdBytes>& m) {
                        ...
                        if (maximum_packet_size_send_ <
                            size<PacketIdBytes>(m)) {
                            pid_man_.release_id(m.packet_id());
                            MQTT_LOG("mqtt_impl", warning)
                                << MQTT_ADD_VALUE(address, this)
                                << "over maximum packet size message removed. "
                                   "packet_id:"
                                << m.packet_id();
                            it = idx.erase(it);
                            return;
                    ...
                    [&](v5::basic_publish_message<PacketIdBytes>& m) {
                        ...
                        any life_keeper;
                        auto msg_lk =
                            apply_topic_alias(m, force_move(life_keeper));
                        if (maximum_packet_size_send_ <
                            size<PacketIdBytes>(std::get<0>(msg_lk))) {
                            pid_man_.release_id(m.packet_id());
                            MQTT_LOG("mqtt_impl", warning)
                        ...
                        if (publish_send_count_.load() == publish_send_max_) {
                            LockGuard<Mutex> lck(publish_send_queue_mtx_);
                            publish_send_queue_.emplace_back(
                                force_move(std::get<0>(msg_lk)), false,
    ...
    void do_sync_write(MessageVariant&& mv) {
        ...
        if (can_send()) {
            on_pre_send();
            total_bytes_sent_ +=
                socket_->write(const_buffer_sequence<PacketIdBytes>(mv), ec);
            // If ec is set as error, the error will be handled by async_read.
            // If `handle_error(ec);` is called here, error_handler would be
            // called twice.
        }
    }

    // Non blocking (async) senders
    void async_send_connect(buffer client_id, optional<buffer> user_name,
        optional<buffer> password, optional<will> const& w,
        std::uint16_t keep_alive_sec, v5::properties props,
        async_handler_t func) {
        shutdown_requested_ = false;
        switch (version_) {
            case protocol_version::v3_1_1:
                do_async_write(v3_1_1::connect_message(keep_alive_sec,
                                   force_move(client_id), clean_session(), w,
                                   force_move(user_name), force_move(password)),
                    force_move(func));
                break;
            case protocol_version::v5:
                ...
                do_async_write(
                    v5::connect_message(keep_alive_sec, force_move(client_id),
                        clean_start(), w, force_move(user_name),
                        force_move(password), force_move(props)),
                    force_move(func));
                break;
            default:
    ...
    void async_send_connack(bool session_present,
        variant<connect_return_code, v5::connect_reason_code> reason_code,
        v5::properties props, async_handler_t func) {
        switch (version_) {
            case protocol_version::v3_1_1: {
    ...
    typename std::enable_if<
        as::is_const_buffer_sequence<ConstBufferSequence>::value>::type
    async_send_publish(packet_id_t packet_id, as::const_buffer topic_name,
        ConstBufferSequence payloads, publish_options pubopts,
        v5::properties props, any life_keeper, async_handler_t func) {
        auto do_async_send_publish = [&](auto msg, auto&& serialize_publish,
                                         auto&& receive_maximum_proc) {
            auto msg_lk = apply_topic_alias(msg, life_keeper);
            if (maximum_packet_size_send_ <
                size<PacketIdBytes>(std::get<0>(msg_lk))) {
            ...
            if (preprocess_publish_message(msg, life_keeper,
                    ...
                    std::forward<decltype(receive_maximum_proc)>(
                        receive_maximum_proc))) {
                do_async_write(force_move(std::get<0>(msg_lk)),
                    [life_keeper = force_move(std::get<1>(msg_lk)), func](
                        error_code ec) {
                        if (func) func(ec);
                    });
            }
        ...
        switch (version_) {
            ...
            case protocol_version::v5:
                do_async_send_publish(
                    v5::basic_publish_message<PacketIdBytes>(packet_id,
                        topic_name, force_move(payloads), pubopts,
                        force_move(props)),
                    &endpoint::on_serialize_v5_publish_message,
                    [this, func](v5::basic_publish_message<PacketIdBytes>&&
                            msg) mutable {
    ...
    void async_send_puback(packet_id_t packet_id, v5::puback_reason_code reason,
        v5::properties props, async_handler_t func) {
        switch (version_) {
            case protocol_version::v3_1_1: {
                auto msg =
                ...
                do_async_write(force_move(msg),
                    [this, self = this->shared_from_this(), packet_id,
                        func = force_move(func)](error_code ec) {
                        if (func) func(ec);
                        on_pub_res_sent(packet_id);
                    });
    ...
    void async_send_pubrec(packet_id_t packet_id, v5::pubrec_reason_code reason,
        v5::properties props, async_handler_t func) {
        switch (version_) {
            case protocol_version::v3_1_1: {
                auto msg =
    ...
    void async_send_pubrel(packet_id_t packet_id, v5::pubrel_reason_code reason,
        v5::properties props, any life_keeper, async_handler_t func) {
        ...
        auto impl = [&](auto msg, auto const& serialize) {
            {
                ...
                auto ret = store_.emplace(
                    packet_id, control_packet_type::pubcomp, msg, life_keeper);
                // publish store is erased when pubrec is received.
                // pubrel store is erased when pubcomp is received.
                // If invalid client send pubrec twice with the same packet id,
                // then send corresponding pubrel twice is a possible
            ...
            do_async_write(
                force_move(msg), [life_keeper = force_move(life_keeper),
                                     func = force_move(func)](error_code ec) {
                    if (func) func(ec);
                });
        };
    ...
    void async_send_pubcomp(packet_id_t packet_id,
        v5::pubcomp_reason_code reason, v5::properties props,
        async_handler_t func) {
        switch (version_) {
            case protocol_version::v3_1_1: {
                auto msg =
    ...
    void async_send_subscribe(
        std::vector<std::tuple<as::const_buffer, subscribe_options>> params,
        packet_id_t packet_id, v5::properties props, async_handler_t func) {
        switch (version_) {
            case protocol_version::v3_1_1: {
                auto msg = v3_1_1::basic_subscribe_message<PacketIdBytes>(
                    force_move(params), packet_id);
    ...
    void async_send_suback(variant<std::vector<suback_return_code>,
                               std::vector<v5::suback_reason_code>>
                               params,
        packet_id_t packet_id, v5::properties props, async_handler_t func) {
        switch (version_) {
    ...
    void async_send_unsubscribe(std::vector<as::const_buffer> params,
        packet_id_t packet_id, v5::properties props, async_handler_t func) {
        switch (version_) {
            case protocol_version::v3_1_1: {
                auto msg = v3_1_1::basic_unsubscribe_message<PacketIdBytes>(
    ...
    void async_send_unsuback(packet_id_t packet_id, async_handler_t func) {
        switch (version_) {
            case protocol_version::v3_1_1: {
                auto msg =
                    v3_1_1::basic_unsuback_message<PacketIdBytes>(packet_id);
    ...
    void async_send_unsuback(std::vector<v5::unsuback_reason_code> params,
        packet_id_t packet_id, v5::properties props, async_handler_t func) {
        switch (version_) {
            case protocol_version::v3_1_1:
                BOOST_ASSERT(false);
    ...
    void async_send_auth(v5::auth_reason_code reason, v5::properties props,
        async_handler_t func) {
        switch (version_) {
            case protocol_version::v3_1_1:
                BOOST_ASSERT(false);
    ...
    void async_send_disconnect(v5::disconnect_reason_code reason,
        v5::properties props, async_handler_t func) {
        switch (version_) {
            case protocol_version::v3_1_1: {
                auto msg = v3_1_1::disconnect_message();
    ...
    void async_send_store(std::function<void()> func) {
        // packet_id has already been registered
        auto g = shared_scope_guard([func = force_move(func)] { func(); });
        LockGuard<Mutex> lck(store_mtx_);
        if (store_.empty()) {
            socket().post([g] {});
            return;
            ...
            MQTT_NS::visit(
                make_lambda_visitor(
                    [&](v3_1_1::basic_publish_message<PacketIdBytes>& m) {
                        ...
                        do_async_write(m, [g](error_code /*ec*/) {});
                        ++it;
                    },
                    [&](v3_1_1::basic_pubrel_message<PacketIdBytes>& m) {
                        MQTT_LOG("mqtt_api", info)
                    ...
                    [&](v5::basic_publish_message<PacketIdBytes>& m) {
                        ...
                        if (publish_send_count_.load() == publish_send_max_) {
                            LockGuard<Mutex> lck(publish_send_queue_mtx_);
                            publish_send_queue_.emplace_back(
                                force_move(std::get<0>(msg_lk)), true,
                                force_move(std::get<1>(msg_lk)));
                        } else {
                            MQTT_LOG("mqtt_impl", trace)
                                << MQTT_ADD_VALUE(address, this)
                            ...
                            ++publish_send_count_;
                            do_async_write(
                                get_basic_message_variant<PacketIdBytes>(
                                    force_move(std::get<0>(msg_lk))),
                                [g, life_keeper = force_move(std::get<1>(
                                        msg_lk))](error_code /*ec*/) {});
                        }
                        ++it;
                    },
                    [&](v5::basic_pubrel_message<PacketIdBytes>& m) {
    ...
    class async_packet {
       public:
        async_packet(
            basic_message_variant<PacketIdBytes> mv, async_handler_t h = {})
            : mv_(force_move(mv)), handler_(force_move(h)) {}
        basic_message_variant<PacketIdBytes> const& message() const {
            return mv_;
        }
        basic_message_variant<PacketIdBytes>& message() { return mv_; }
    ...
    struct write_completion_handler {
        write_completion_handler(std::shared_ptr<this_type> self,
            async_handler_t func, std::size_t num_of_messages,
            std::size_t expected)
        ...
        void operator()(error_code ec, std::size_t bytes_transferred) const {
            func_(ec);
            ...
            if (bytes_to_transfer_ != bytes_transferred) {
                ...
                throw write_bytes_transferred_error(
                    bytes_to_transfer_, bytes_transferred);
            }
            if (!self_->queue_.empty()) {
                self_->do_async_write();
            }
    ...
    void do_async_write() {
        ...
        std::size_t iterator_count =
            (max_queue_send_count_ == 0)
                ? queue_.size()
                : std::min(max_queue_send_count_, queue_.size());
        auto const& start = queue_.cbegin();
        auto end =
            std::next(start, boost::numeric_cast<difference_t>(iterator_count));
        ...
        for (auto it = start; it != end; ++it) {
            ...
            // If we hit the byte limit, we don't include this buffer for this
            // send.
            if (max_queue_send_size_ != 0 &&
                max_queue_send_size_ < total_bytes + size) {
                end = it;
                iterator_count =
                    boost::numeric_cast<std::size_t>(std::distance(start, end));
                break;
            }
            total_bytes += size;
        ...
        socket_->async_write(force_move(buf),
            write_completion_handler(
                this->shared_from_this(),
                [handlers = force_move(handlers)](error_code ec) {
                    for (auto const& h : handlers) {
                        if (h) h(ec);
                    }
                },
                iterator_count, total_bytes));
    }

    void do_async_write(
        basic_message_variant<PacketIdBytes> mv, async_handler_t func) {
        // Move this job to the socket's strand so that it can be queued without
        // mutexes.
        socket_->post(
            [this, self = this->shared_from_this(), mv = force_move(mv),
                func = force_move(func)]() mutable {
                if (can_send()) {
                    queue_.emplace_back(force_move(mv), force_move(func));
                    // Only need to start async writes if there was nothing in
                    // the queue before the above item.
                    if (queue_.size() > 1) return;
                    do_async_write();
                } else {
                    // offline async publish is successfully finished, because
    ...
    void set_pingresp_timer() {
        ...
        std::weak_ptr<this_type> wp(
            std::static_pointer_cast<this_type>(this->shared_from_this()));
        tim_pingresp_.async_wait([wp = force_move(wp)](error_code ec) {
            if (auto sp = wp.lock()) {
                sp->tim_pingresp_set_ = false;
                if (!ec) {
                    sp->socket().post([sp] { sp->force_disconnect(); });
                }
            }
        });
    }
    ...
    void send_publish_queue_one() {
        ...
        --publish_send_count_;
        auto entry = [&]() -> optional<publish_send_queue_elem> {
            LockGuard<Mutex> lck(publish_send_queue_mtx_);
        ...
        if (entry.value().async) {
            do_async_write(force_move(entry.value().message),
    ...
    bool can_send() const { return connected_ && !shutdown_requested_; }

    static optional<topic_alias_t> get_topic_alias_from_prop(
        v5::property_variant const& prop) {
        optional<topic_alias_t> val;
        v5::visit_prop(
            prop, [&val](v5::property::topic_alias const& p) { val = p.val(); },
            [](auto&&) {});
        return val;
    }

    static optional<topic_alias_t> get_topic_alias_from_props(
        v5::properties const& props) {
        optional<topic_alias_t> val;
        v5::visit_props(
            props,
            [&val](v5::property::topic_alias const& p) { val = p.val(); },
            [](auto&&) {});
        return val;
    }
   ...
   protected:
    ...
    ~endpoint() {
        MQTT_LOG("mqtt_impl", trace)
            << MQTT_ADD_VALUE(address, this) << "endpoint destroy";
    }
   ...
   private:
    ...
    std::chrono::steady_clock::duration pingresp_timeout_ =
    ...
    struct publish_send_queue_elem {
        publish_send_queue_elem(basic_message_variant<PacketIdBytes> message,
            bool async, any life_keeper = any())
