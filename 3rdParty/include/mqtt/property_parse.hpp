...
inline optional<property_variant> parse_one(buffer& buf) {
    ...
    try {
        ...
        switch (id) {
            case id::payload_format_indicator: {
                ...
                auto p = payload_format_indicator(
                    buf.begin(), std::next(buf.begin(), 1));
                buf.remove_prefix(1);
                return property_variant(p);
            } break;
            case id::message_expiry_interval: {
                if (buf.size() < 4) return nullopt;
                auto p = message_expiry_interval(
                    buf.begin(), std::next(buf.begin(), 4));
                buf.remove_prefix(4);
                return property_variant(p);
            } break;
            ...
            case id::session_expiry_interval: {
                if (buf.size() < 4) return nullopt;
                auto p = session_expiry_interval(
                    buf.begin(), std::next(buf.begin(), 4));
                buf.remove_prefix(4);
                return property_variant(p);
            } break;
            ...
            case id::server_keep_alive: {
                if (buf.size() < 2) return nullopt;
                auto p =
                    server_keep_alive(buf.begin(), std::next(buf.begin(), 2));
                buf.remove_prefix(2);
                return property_variant(p);
            } break;
            ...
            case id::request_problem_information: {
                if (buf.size() < 1) return nullopt;
                auto p = request_problem_information(
                    buf.begin(), std::next(buf.begin(), 1));
                buf.remove_prefix(1);
                return property_variant(p);
            } break;
            case id::will_delay_interval: {
                if (buf.size() < 4) return nullopt;
                auto p =
                    will_delay_interval(buf.begin(), std::next(buf.begin(), 4));
                buf.remove_prefix(4);
                return property_variant(p);
            } break;
            case id::request_response_information: {
                if (buf.size() < 1) return nullopt;
                auto p = request_response_information(
                    buf.begin(), std::next(buf.begin(), 1));
                buf.remove_prefix(1);
                return property_variant(p);
            } break;
            ...
            case id::receive_maximum: {
                if (buf.size() < 2) return nullopt;
                auto p =
                    receive_maximum(buf.begin(), std::next(buf.begin(), 2));
                buf.remove_prefix(2);
                return property_variant(p);
            } break;
            case id::topic_alias_maximum: {
                if (buf.size() < 2) return nullopt;
                auto p =
                    topic_alias_maximum(buf.begin(), std::next(buf.begin(), 2));
                buf.remove_prefix(2);
                return property_variant(p);
            } break;
            case id::topic_alias: {
            ...
            case id::retain_available: {
                if (buf.size() < 1) return nullopt;
                auto p =
                    retain_available(buf.begin(), std::next(buf.begin(), 1));
                buf.remove_prefix(1);
                return property_variant(p);
            } break;
            ...
            case id::maximum_packet_size: {
                if (buf.size() < 4) return nullopt;
                auto p =
                    maximum_packet_size(buf.begin(), std::next(buf.begin(), 4));
                buf.remove_prefix(4);
                return property_variant(p);
            } break;
            case id::wildcard_subscription_available: {
                if (buf.size() < 1) return nullopt;
                auto p = wildcard_subscription_available(
                    buf.begin(), std::next(buf.begin(), 1));
                buf.remove_prefix(1);
                return property_variant(p);
            } break;
            case id::subscription_identifier_available: {
                if (buf.size() < 1) return nullopt;
                auto p = subscription_identifier_available(
                    buf.begin(), std::next(buf.begin(), 1));
                buf.remove_prefix(1);
                return property_variant(p);
            } break;
            case id::shared_subscription_available: {
                if (buf.size() < 1) return nullopt;
                auto p = shared_subscription_available(
                    buf.begin(), std::next(buf.begin(), 1));
                buf.remove_prefix(1);
                return property_variant(p);
            } break;
...
inline std::vector<property_variant> parse(buffer buf) {
