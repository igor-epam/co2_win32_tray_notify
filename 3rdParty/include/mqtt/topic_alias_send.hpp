...
class topic_alias_send {
   public:
    topic_alias_send(topic_alias_t max) : max_{max}, va_{min_, max_} {}

    void insert_or_update(string_view topic, topic_alias_t alias) {
        MQTT_LOG("mqtt_impl", trace)
            << MQTT_ADD_VALUE(address, this) << "topic_alias_send insert"
        ...
        if (it == idx.end() || it->alias != alias) {
            idx.emplace_hint(it, std::string(topic), alias,
    ...
    std::string find(topic_alias_t alias) {
        MQTT_LOG("mqtt_impl", trace)
            << MQTT_ADD_VALUE(address, this) << "find_topic_by_alias"
            << " alias:" << alias;
        ...
        idx.modify(
            it, [&](entry& e) { e.tp = std::chrono::steady_clock::now(); },
            [](auto&) { BOOST_ASSERT(false); });
        return it->topic;
    }
    ...
    optional<topic_alias_t> find(string_view topic) const {
        MQTT_LOG("mqtt_impl", trace)
            << MQTT_ADD_VALUE(address, this) << "find_alias_by_topic"
            << " topic:" << topic;
    ...
    void clear() {
        MQTT_LOG("mqtt_impl", info)
            << MQTT_ADD_VALUE(address, this) << "clear_topic_alias";
        aliases_.clear();
        va_.clear();
    }
   ...
   private:
    ...
    struct entry {
        ...
        string_view get_topic_as_view() const { return topic; }
    ...
    using mi_topic_alias = mi::multi_index_container<entry,
        mi::indexed_by<
            mi::ordered_unique<mi::tag<tag_alias>,
                BOOST_MULTI_INDEX_MEMBER(entry, topic_alias_t, alias)>,
            mi::ordered_unique<mi::tag<tag_topic_name>,
                BOOST_MULTI_INDEX_CONST_MEM_FUN(
                    entry, string_view, get_topic_as_view)>,
            mi::ordered_non_unique<mi::tag<tag_tp>,
