...
struct subscribe_entry {
    subscribe_entry(
        buffer share_name, buffer topic_filter, subscribe_options subopts)
    ...
    subscribe_entry(buffer topic_filter, subscribe_options subopts)
        : topic_filter{force_move(topic_filter)}, subopts{subopts} {}
    ...
    // contain a ShareName that is at least one character long [MQTT-4.8.2-1].
    buffer share_name;
    buffer topic_filter;
