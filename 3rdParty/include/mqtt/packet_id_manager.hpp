...
class packet_id_manager {
   ...
   public:
     ...
     * @return packet id
     */
    optional<packet_id_t> acquire_unique_id() { return va_.allocate(); }
     ...
     */
    bool register_id(packet_id_t packet_id) { return va_.use(packet_id); }
     ...
     */
    void release_id(packet_id_t packet_id) { va_.deallocate(packet_id); }
   ...
   private:
    value_allocator<packet_id_t> va_{
        1, std::numeric_limits<packet_id_t>::max()};
};
