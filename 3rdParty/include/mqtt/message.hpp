...
class basic_header_packet_id_message {
   public:
     ...
     * @brief Create empty header_packet_id_message.
     */
    basic_header_packet_id_message(control_packet_type type, std::uint8_t flags,
        typename packet_id_type<PacketIdBytes>::type packet_id)
        : message_{static_cast<char>(make_fixed_header(type, flags)),
              PacketIdBytes} {
    ...
    basic_header_packet_id_message(Iterator b, Iterator e) {
        if (std::distance(b, e) != 2 + PacketIdBytes)
   ...
   protected:
    boost::container::static_vector<char, 2 + PacketIdBytes> const& message()
        const {
        return message_;
    }
...
struct basic_puback_message
    : detail_v3_1_1::basic_header_packet_id_message<PacketIdBytes> {
...
struct basic_pubrel_message
    ...
    basic_pubrel_message(string_view buf) : base(buf.begin(), buf.end()) {}
    ...
    typename packet_id_type<PacketIdBytes>::type packet_id() const {
        return make_packet_id<PacketIdBytes>::apply(
            std::next(base::message().begin(), 2), base::message().end());
    }
};
...
struct basic_pubcomp_message
    ...
    basic_pubcomp_message(
        typename packet_id_type<PacketIdBytes>::type packet_id)
        : detail_v3_1_1::basic_header_packet_id_message<PacketIdBytes>(
              control_packet_type::pubcomp, 0b0000, packet_id) {}
};
...
struct basic_unsuback_message
    ...
    basic_unsuback_message(
        typename packet_id_type<PacketIdBytes>::type packet_id)
        : detail_v3_1_1::basic_header_packet_id_message<PacketIdBytes>(
              control_packet_type::unsuback, 0b0000, packet_id) {}
};
...
class connect_message {
   public:
    connect_message(std::uint16_t keep_alive_sec, buffer client_id,
        bool clean_session, optional<will> w, optional<buffer> user_name,
        optional<buffer> password)
        : fixed_header_(static_cast<char>(
...
class basic_publish_message {
   public:
    ...
    basic_publish_message(buffer buf) {
        ...
        std::copy(buf.begin(),
            std::next(buf.begin(),
                static_cast<string_view::difference_type>(consumed)),
        ...
        auto topic_name_length = make_uint16_t(
            topic_name_length_buf_.begin(), topic_name_length_buf_.end());
        buf.remove_prefix(2);
   ...
   public:
    ...
    std::vector<as::const_buffer> const_buffer_sequence() const {
        ...
        for (auto const& e : entries_) {
            ret.emplace_back(as::buffer(e.topic_name_length_buf_.data(),
                e.topic_name_length_buf_.size()));
    ...
    std::string continuous_buffer() const {
        ...
        for (auto const& e : entries_) {
            ret.append(e.topic_name_length_buf_.data(),
                e.topic_name_length_buf_.size());
...
class basic_suback_message {
   public:
    basic_suback_message(std::vector<suback_return_code> params,
        typename packet_id_type<PacketIdBytes>::type packet_id)
        ...
        // TODO: We should be able to simply static-cast params.data() into a
        // char*.
        entries_.reserve(params.size());
        for (auto e : params) {
