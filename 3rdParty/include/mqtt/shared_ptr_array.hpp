 ...
 * `std::make_shared<char[]>(size)` is used.
 *      - It can allocate an array of characters and the control block in a
 * single allocation.
 ...
 * `boost::make_shared<char[]>(size)` is used.
 *      - It can allocate an array of characters and the control block in a
 * single allocation.
 */
