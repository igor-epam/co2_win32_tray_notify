...
class topic_alias_recv {
   public:
    topic_alias_recv(topic_alias_t max) : max_{max} {}

    void insert_or_update(string_view topic, topic_alias_t alias) {
        MQTT_LOG("mqtt_impl", trace)
            << MQTT_ADD_VALUE(address, this) << "topic_alias_recv insert"
        ...
        if (it == aliases_.end() || it->alias != alias) {
            ...
            aliases_.modify(
                it, [&](entry& e) { e.topic = std::string{topic}; },
   ...
   private:
    ...
    using mi_topic_alias = mi::multi_index_container<entry,
        mi::indexed_by<mi::ordered_unique<BOOST_MULTI_INDEX_MEMBER(
            entry, topic_alias_t, alias)> > >;
